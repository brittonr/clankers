...
- [ ] **Session Forking** — Branch conversations to explore alternatives: `/fork`, `/rewind`, `/branches`, `/switch`, `/label`, `/compare`, `/merge`, `/cherry-pick` commands (phases 1-9, 11 ✅). Branch panel, switcher overlay, comparison view, merge, cherry-pick all done. Remaining: interactive merge UI, keyboard shortcuts polish, edge cases, docs. (`openspec/changes/session-forking/`)

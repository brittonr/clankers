...
- [x] **Process Monitor** — Background `sysinfo`-based process tracker. Registers PIDs from bash/subagent/delegate tools, polls CPU/RSS every 2s, detects exits, emits `AgentEvent` lifecycle events. `procmon` tool for agent self-inspection (list/summary/inspect/history). TUI process panel with sparklines, sort cycling, detail view, status bar aggregate. All 4 phases complete ✅. (`openspec/changes/proc-mon/`)

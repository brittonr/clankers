...
- [ ] **Multi-Model Conversations** — Dynamic model routing within a session: complexity-based auto-selection, cost tracking with budget thresholds, agent-initiated model switching, orchestration patterns (propose/validate, plan/execute). Builds on existing `ModelRole` + `clankers-router`. Phases 1-5 (routing, cost tracking, agent switching, TUI display, cost tool) ✅. (`openspec/changes/multi-model/`)

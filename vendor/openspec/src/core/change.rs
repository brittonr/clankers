...
/// Parse tasks from content string (pure function)
/// Returns None if no task checkboxes found
pub fn parse_task_progress_content(content: &str) -> Option<TaskProgress> {
...
/// Parse change metadata from .openspec.yaml content (pure function)
pub fn parse_change_meta(content: &str) -> (String, String) {
...
fn parse_task_progress(path: &Path) -> Option<TaskProgress> {
    ...
    parse_task_progress_content(&content)
}
...
mod tests {
    ...
    #[test]
    fn test_parse_task_progress_content_none() {
        let progress = parse_task_progress_content("# Tasks\n\nNo checkboxes here");
        assert!(progress.is_none());
    }

    #[test]
    fn test_parse_task_progress_content_partial() {
        let content = "- [x] Done\n- [ ] Todo\n- [X] Also done";
    ...
    #[test]
    fn test_parse_change_meta() {
        let content = "schema: spec-driven\ncreated: 2026-03-03T10:00:00Z\n";
        let (schema, created) = parse_change_meta(content);
        assert_eq!(schema, "spec-driven");
        assert_eq!(created, "2026-03-03T10:00:00Z");
    }

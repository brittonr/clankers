...
/// Parse spec content from a string (pure function)
pub fn parse_spec_content(content: &str, domain: &str) -> Option<Spec> {
    ...
    Some(Spec {
        ...
        file_path: PathBuf::from(format!("{}.md", domain)), // placeholder path
        purpose,
        requirements,
    })
}
...
/// Detect requirement strength from text (pure function)
pub fn detect_strength(text: &str) -> RequirementStrength {
...
/// Parse GIVEN/WHEN/THEN scenarios from text (pure function)
pub fn parse_scenarios(text: &str) -> Vec<Scenario> {
...
mod tests {
    ...
    #[test]
    fn test_parse_spec_content() {
        let content = "## Purpose\nThis is a test spec\n\n## Authentication\nThe system MUST authenticate users.\n\nGIVEN a user\nWHEN they provide credentials\nTHEN they are authenticated";
        let spec = parse_spec_content(content, "auth").unwrap();

        assert_eq!(spec.domain, "auth");
        assert_eq!(spec.purpose.as_ref().unwrap(), "This is a test spec");
        assert_eq!(spec.requirements.len(), 1);
        assert_eq!(spec.requirements[0].heading, "Authentication");
        assert_eq!(spec.requirements[0].strength, RequirementStrength::Must);

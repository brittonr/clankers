    ...
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in
      {
        ...
        // pkgs.lib.optionalAttrs pkgs.stdenv.isLinux {
          ...
          # Session recovery: create sessions → stop → restart → verify
          # suspended sessions survive, then crash recovery (SIGKILL → restart).
          vm-session-recovery = pkgs.testers.runNixOSTest {
            ...
            testScript = ''
              ...
              # Verify daemon status shows 2 sessions
              ...
              # Verify the redb catalog file exists
              ...
              # Status should show the suspended count
              status_out = machine.succeed("clankers daemon status")
              ...
              # Restart — stale active entries should become suspended
              machine.succeed(
                  "clankers daemon start --heartbeat 0 "
                  "> /tmp/daemon3.log 2>&1 &"
              )
              ...
              # Sessions should survive crash recovery
              ps_out = machine.succeed("clankers ps")

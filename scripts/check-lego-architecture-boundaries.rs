...
const AGENT_CONFIG_ADAPTER: &str = "src/agent_config.rs";
...
fn run() -> Result<PathBuf, String> {
    ...
    let receipt = json!({
        ...
        "artifact_hashes": [
            ...
            hash_artifact(Path::new(AGENT_CONFIG_ADAPTER))?,
...
fn root_dependency_owner(target_crate: &str) -> Result<(&'static str, &'static str, &'static str), String> {
    match target_crate {
        "clanker-message" => Ok(("edge projection: shared message DTO", "src/runtime_services.rs", "remain shared DTO only; no root-owned message policy")),
        "clanker-router" => Ok(("app-edge wiring: desktop provider routing", "src/runtime_services.rs", "route through ProviderRouterService and provider bridge")),
        "clanker-tui-types" => Ok(("edge projection: display DTO", "src/modes/event_loop_runner", "keep rendering rules in TUI/display adapters")),
        "clankers-agent" => Ok(("temporary policy: desktop agent construction", "src/agent.rs", "move agent execution construction behind runtime/controller adapters")),
        "clankers-agent-defs" => Ok(("app-edge wiring: CLI agent profile loading", "src/cli.rs", "profile semantics stay in agent-defs crate")),
        "clankers-artifacts" => Ok(("app-edge wiring: artifact command shell", "src/commands", "artifact policy remains in artifacts crate")),
        "clankers-autoresearch" => Ok(("app-edge wiring: autoresearch mode dispatch", "src/modes", "research policy remains in autoresearch crate")),
        "clankers-config" => Ok(("app-edge wiring: desktop config loading", "src/config", "neutral config stays in clankers-config core DTOs")),
        "clankers-controller" => Ok(("adapter exception: session orchestration adapter", "src/modes/event_loop_runner", "controller owns command lifecycle only")),
        "clankers-core" => Ok(("temporary policy: control-state bridge", "src/modes/session_command_policy.rs", "core reducer policy remains in clankers-core")),
        "clankers-db" => Ok(("temporary policy: desktop storage wiring", "src/runtime_services.rs", "move storage access behind runtime/session stores")),
        "clankers-hooks" => Ok(("app-edge wiring: desktop hook wiring", "src/modes", "hook policy remains in hooks crate")),
        "clankers-matrix" => Ok(("app-edge wiring: optional Matrix mode", "src/commands/daemon.rs", "Matrix protocol remains in matrix crate")),
        "clankers-model-selection" => Ok(("temporary policy: desktop model selection", "src/runtime_services.rs", "move model routing state behind provider/runtime service")),
        "clankers-nix" => Ok(("app-edge wiring: Nix tool/mode shell", "src/tools", "Nix semantics remain in clankers-nix crate")),
        "clankers-plugin" => Ok(("app-edge wiring: plugin host wiring", "src/plugin", "plugin runtime policy remains in plugin host facade")),
        "clankers-procmon" => Ok(("adapter exception: process-monitor tool shell", "src/tools/process", "process policy remains in procmon/tool adapter")),
        "clankers-prompts" => Ok(("app-edge wiring: desktop prompt source wiring", "src/runtime_prompt.rs", "prompt assembly stays in prompt/runtime services")),
        "clankers-protocol" => Ok(("edge projection: daemon protocol", "src/modes/daemon", "protocol frames stay at transport edge")),
        "clankers-provider" => Ok(("temporary policy: desktop provider construction", "src/runtime_services.rs", "provider shaping stays in provider/router bridge")),
        "clankers-runtime" => Ok(("app-edge wiring: runtime facade composition", "src/runtime_services.rs", "runtime services remain explicit embeddable bricks")),
        "clankers-session" => Ok(("temporary policy: desktop session storage", "src/modes/session_setup.rs", "move storage policy behind SessionStore/ledger DTOs")),
        "clankers-skills" => Ok(("app-edge wiring: desktop skill discovery", "src/runtime_services.rs", "skill discovery stays behind explicit SkillStore roots")),
        "clankers-tool-host" => Ok(("adapter exception: tool host DTO bridge", "src/tools", "tool execution policy stays in tool-host/adapters")),
        "clankers-tts" => Ok(("app-edge wiring: optional voice mode", "src/commands", "TTS policy stays in tts crate")),
        "clankers-tui" => Ok(("edge projection: display shell", "src/modes/event_loop_runner", "rendering remains in TUI crate")),
        "clankers-ucan" => Ok(("app-edge wiring: capability auth shell", "src/capability", "authorization policy stays in UCAN/capability gate")),
        "clankers-util" => Ok(("temporary policy: desktop utility wiring", "src", "replace reusable utility policy with owning brick APIs or edge-only helpers")),
        "clankers-zellij" => Ok(("app-edge wiring: optional zellij mode", "src/commands", "zellij integration remains edge adapter")),
...
fn agent_turn_ports_signature() -> Result<Value, String> {
    let agent_config_file = read_rust_file(AGENT_CONFIG_ADAPTER)?;
    ...
    let agent_config = &agent_config_file.source;
    ...
    require_contains(agent_config, "agent_settings_from_config", "agent runtime settings app-edge adapter")?;
    require_contains(agent_config, "agent_model_roles_from_config", "agent model-role app-edge adapter")?;
    forbid_contains(
        agent_lib,
        "clankers_config",
    ...
    forbid_contains(
        builder,
        "clankers_config",
        "agent builder concrete config import after DTO drain",
    )?;
    require_contains(
        compaction,
        "pub struct AutoCompactSettings",
        "agent-owned auto-compaction settings DTO",
    )?;
    require_contains(
        context,
    ...
    forbid_contains(
        compaction,
        "clankers_config::",
        "auto-compaction reusable policy concrete config import",
    )?;
    forbid_contains(
        context,
        "clankers_config",

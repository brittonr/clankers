...
const FIXTURE_ROOT: &str = "scripts/fixtures/cairn-review-gates";
const ERROR_EXIT: u8 = 1;

const GUIDANCE_PATH: &str = "docs/src/reference/cairn-review-gates.md";
const OPERATOR_GUIDE_PATH: &str = "docs/src/reference/cairn-review-gates.md";
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => {
            println!("ok: cairn review-gate fixtures passed");
            ExitCode::SUCCESS
        }
        Err(error) => {
...
fn run() -> Result<(), String> {
    ...
    fixture_dirs.sort();
    require(!fixture_dirs.is_empty(), "no cairn review-gate fixtures found")?;
...
fn verify_guidance_and_wiring() -> Result<(), String> {
    ...
    for required in [
        ...
        "scripts/check-cairn-review-gates.rs",
    ...
    require(
        flake.contains("cairn-review-gates"),
        "flake.nix must expose checks.<system>.cairn-review-gates",
    )?;
    require(
        flake.contains("check-cairn-review-gates.rs"),
        "flake.nix cairn review gate check must run the Rust checker",
    )?;

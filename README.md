...
./scripts/test-harness.sh full         # fmt, tests, clippy, repo rails, tigerstyle, live aspen2 Qwen, BG-process TUI dogfood
./scripts/test-harness.sh soak         # repeated dogfood rails for flake hunting (CLANKERS_SOAK_ITERATIONS or arg controls count)
./scripts/test-harness.sh vm           # all Linux Nix VM checks
./scripts/test-harness.sh ci           # exact nix flake check gate
```

Install the local Tigerstyle pre-commit gate with:
...
The harness writes machine-readable results to `target/test-harness/results.json`, a Markdown summary to `target/test-harness/summary.md`, JUnit XML for CI collectors to `target/test-harness/junit.xml`, and per-step logs under `target/test-harness/logs/`. Use `CLANKERS_TEST_DRY_RUN=1` to inspect a tier without running it, or `CLANKERS_TEST_RESULT_DIR=path/to/results` to redirect all harness reports. Release-readiness assertions are Rust tests discoverable by `cargo nextest`; `tests/e2e/run-tests.sh` is only a compatibility wrapper. The credential-free E2E rail is `cargo nextest run -p clankers --test readiness_e2e --no-fail-fast`, while opt-in live/VM/flake rails live in `tests/readiness_opt_in.rs` behind `CLANKERS_RUN_LIVE_READINESS=1`, `CLANKERS_RUN_VM_READINESS=1`, and `CLANKERS_RUN_FLAKE_READINESS=1`. The maintained `full` gate now includes the primary `aspen2-qwen36` live readiness step after deterministic local rails, then runs `dogfood bg-process-tui` so the real TUI `/layout toggle bg` and `Spawned/BG (N active)` background-process panel stay in the normal readiness profile. `nix flake check` includes the credential-free `e2e-fake` check, which now exercises the nextest-owned deterministic fake-provider E2E tier through the packaged `clankers` binary. The embedded SDK receipt rail is guarded by the pure Nix check `nix build .#checks.$(nix eval --raw --impure --expr builtins.currentSystem).embedded-sdk-release-receipt`; the maintained acceptance rail itself is `scripts/check-embedded-agent-sdk.rs` with the legacy `.sh` wrapper only delegating to the Rust script. The release-readiness checklist lives at `docs/src/reference/release-readiness.md`; include `./scripts/test-harness.sh live aspen2-qwen36` as the preferred Lemonade/Qwen3.6 live runtime smoke when the local endpoint is reachable, `./scripts/test-harness.sh dogfood streaming-tokens` when debugging standalone token/thinking streaming, `./scripts/test-harness.sh dogfood daemon-attach-streaming-abort` when debugging daemon/attach mid-stream abort-and-follow-up handling, or `./scripts/test-harness.sh dogfood bg-process-tui` when debugging just the operator-visible background-process TUI seam. The optional live aspen2 check is not part of normal pure evaluation; from the repo root run `CLANKERS_ENABLE_LIVE_CHECKS=1 nix build --impure --no-link .#checks.$(nix eval --raw --impure --expr builtins.currentSystem).live-aspen2-qwen36 --option sandbox false -L`. It expects the sibling rats checkout at `../rats` by default; set `CLANKERS_LIVE_RATS_DIR=/path/to/rats` to override. The test self-skips when the aspen2 endpoint/model is unavailable.
...
./scripts/test-harness.sh dogfood bg-process-tui                 # real TUI background-process panel dogfood receipt
./scripts/test-harness.sh dogfood streaming-tokens               # real TUI token/thinking streaming recording receipt
./scripts/test-harness.sh dogfood daemon-attach-streaming-abort  # real attach TUI abort/follow-up while daemon stream is active
./scripts/test-harness.sh soak streaming 2                     # repeat streaming dogfood rails twice for flake hunting
./scripts/test-harness.sh soak daemon-attach 3                 # repeat daemon/attach dogfood rails three times
...
`openai-codex/gpt-5.5` for ChatGPT subscription Codex and `openai/gpt-4o` for API-key OpenAI.
...
clankers batch run --input prompts.jsonl --output eval.jsonl --format eval-jsonl --execution daemon --run-id nightly-smoke --resume
```

Input is local JSONL: each line is an object with a non-empty `prompt`, optional string `id`, and optional object `metadata`. Outputs are local JSONL, ShareGPT-style JSON, or eval JSONL. `--execution daemon` assigns deterministic per-job session ids and resumes each prompt through the normal clankers session path so manifests can record session provenance; `--resume` reads the sidecar manifest and skips completed job ids. Remote `http://`, `https://`, or `s3://` inputs/outputs are rejected, concurrency is bounded, result ordering is stable, and replay/debug metadata records counts, prompt sizes, model/session handles, redaction status, and simple objective receipts rather than raw prompts or credentials.

## Built-in Tools

Core: `read`, `write`, `edit`, `patch`, `execute_code`, `process`, `bash`, `grep`, `find`, `ls`, `ask`, `commit`, `web`, `nix`

Steel eval: the `steel_eval` built-in is published under ordinary default settings. It uses the pure default Steel runtime profile: no ambient host functions, no session capabilities, zero host-call budget, and no filesystem, process, network, provider, credential, daemon/session, TUI, mutation, or native-tool authority. `steel_eval` is only local Scheme evaluation through the Rust-owned Steel wrapper and deterministic receipt path; it is separate from Steel turn planning and from mutation-capable Steel runs. Operators who want to hide the tool can set `steelEval.enabled = false` in settings. Steel turn planning now defaults to the bundled reviewed `steel.host.plan_turn` profile/script; set `steelTurnPlanning.enabled = false` to opt out and keep Rust-native planning. Receipts expose safe metadata such as profile id, status/issue code, output length/hash material, and runtime receipt fields without granting additional authority. See [`docs/src/reference/steel-scheme-runtime.md`](docs/src/reference/steel-scheme-runtime.md).
...
External memory providers: the Specialty `external_memory` tool is published only when `externalMemory.enabled` validates. The local provider searches the clankers memory database. The HTTP provider stays disabled by default but is supported when explicitly configured with `provider = "http"`, `endpoint`, `credentialEnv`, optional `timeoutMs`, and bounded `maxResults`; it fails before network contact when credentials are absent, sends only bounded search requests, and returns replay-safe metadata. `injectIntoPrompt` is opt-in policy state only here: explicit tool calls can return remote results, but automatic prompt injection remains off unless future prompt assembly consumes that flag.

Browser automation: the Specialty `browser` tool is published when `browserAutomation.enabled` is true with a valid local CDP endpoint or browser binary. The CDP backend supports stateful `navigate`, `snapshot`/`current_url`, `click`, `type`/`fill`, `screenshot`, `evaluate`, and `close`; navigation origins, JavaScript evaluation, and screenshots are policy-gated before backend contact. Replay/debug metadata records action/status/backend/session/page labels and sanitized errors, not raw CDP URLs, headers, credentials, or environment values.

Working-directory checkpoints: `clankers checkpoint create [--label <LABEL>]`, `clankers checkpoint list`, and `clankers checkpoint rollback <CHECKPOINT_ID> --yes` snapshot and restore local git checkout files using `.git/clankers-checkpoints`. Agents can use the Specialty `checkpoint` tool for the same local git-backed create/list/rollback surface. File-mutating core tools (`write`, `edit`, and `patch`) create a local clankers checkpoint before mutating files in a git checkout and attach the safe checkpoint receipt to tool-result metadata; outside git they skip the automatic guard rather than inventing a remote snapshot backend. The first pass is local-only: non-git directories for explicit checkpoint commands, remote checkpoint stores, submodule recursion, and rollback without explicit confirmation return actionable errors. Replay/debug metadata records action/status/backend/repo/checkpoint id/counts and sanitized errors, not raw diffs or file contents.

Tool gateway delivery: `clankers gateway status [--json]` reports the local gateway boundary, `clankers gateway validate --toolsets <LIST> [--deliver <TARGET>] [--json]` validates local/session/active-Matrix delivery and toolset names, `clankers gateway deliver --artifact-type <file|media|scheduled-output> [--path <PATH>] [--deliver <TARGET>] [--outbox <PATH>] [--matrix-active] [--json]` routes an artifact through the shared adapter boundary and can record a safe outbox attempt, `clankers gateway delivery-status --outbox <PATH> [--json]` lists recorded attempts, `clankers gateway retry --outbox <PATH> --attempt-id <ID> [--json]` retries only by attempt id, and `clankers gateway deliver-receipt ...` remains a compatibility receipt-only path. Agents can use the Specialty `tool_gateway` tool for the same status/validate/deliver_receipt surface. Standalone and daemon tool rebuilding use the shared gateway policy helpers for active toolsets plus disabled-tool filtering. Remote/platform delivery, Matrix delivery outside an active bridge, webhooks, cloud storage targets, and credential/header delivery return explicit unsupported errors. Replay/debug metadata records only safe labels, basename-only paths, counts, handles, statuses, and error classes.

Voice/STT validation and live receipts: `clankers voice status [--json]` reports voice support boundaries, `clankers voice validate --input <SOURCE> [--reply <text|tts|none>] [--json]` validates a local file or unsupported input, `clankers voice start --enable [--input microphone] [--auto-submit] [--json]` emits an explicit start-capture receipt without retaining raw audio, `clankers voice stop [--json]` closes the capture handle, and `clankers voice submit-transcript --transcript <TEXT> [--reply <text|tts|none>] [--auto-submit] [--json]` prepares an accepted transcript for the normal session prompt path. Agents can use the Specialty `voice_mode` tool for the same status/validate/start_capture/stop_capture/submit_transcript surface. Live capture remains explicit and local-first: no background microphone capture starts by default, cloud STT is disabled by policy, and replay/debug metadata records safe input labels, capture status, reply mode, counts, and transcript digests rather than raw audio, transcripts, full paths, URLs, credentials, or Matrix payloads.

SOUL/personality prompts: local `SOUL.md` is now part of system-prompt assembly when present in `.clankers/SOUL.md`, project-root `SOUL.md`, or `~/.clankers/agent/SOUL.md`. Set `CLANKERS_PERSONALITY_PRESET=<name>` to include `.clankers/personality/<name>.md` or `~/.clankers/agent/personality/<name>.md`; preset names are limited to ASCII letters, digits, `.`, `_`, and `-`. Set `CLANKERS_DISABLE_SOUL_PERSONALITY=1` to omit SOUL and preset prompt sections entirely. The SOUL/personality layer is inserted after SYSTEM/APPEND_SYSTEM and before AGENTS.md/CLAUDE.md so project operating rules can still refine behavior. `clankers soul status [--json]` and `clankers soul validate [--soul <PATH|discover>] [--personality <NAME>] [--json]` keep the non-mutating policy-validation surface, and agents can use the Specialty `soul_personality` tool for the same status/validate checks. Remote/cloud persona fetching, command hooks, encrypted/secret bundles, raw prompt/personality persistence, and autonomous self-modifying personality changes return explicit unsupported errors. Replay/debug metadata records safe source kind/label, optional preset name, support flag, path hash, byte count, precedence, and sanitized error category/message, not raw SOUL contents, full paths, URLs, headers, commands, credentials, or prompt text.

...
Tool gateway validation and delivery receipts: `clankers gateway status [--json]` reports the local gateway boundary, `clankers gateway validate --toolsets <LIST> [--deliver <TARGET>] [--json]` validates local/session delivery and toolset names, and `clankers gateway deliver-receipt --artifact-type <file|media|scheduled-output> [--path <PATH>] [--deliver <TARGET>] [--json]` emits a safe receipt without sending data. Agents can use the Specialty `tool_gateway` tool for the same status/validate/deliver_receipt surface. Standalone and daemon tool rebuilding use the shared gateway policy helpers for active toolsets plus disabled-tool filtering. Remote/platform delivery, Matrix delivery outside an active bridge, webhooks, cloud storage targets, and credential/header delivery return explicit unsupported errors. Replay/debug metadata records only safe labels, basename-only paths, counts, handles, statuses, and error classes.

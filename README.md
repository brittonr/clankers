...
Specialty: `review`, `todo`, `cost`, `schedule`, `browser` (when `browserAutomation.enabled`), `external_memory` (when `externalMemory.enabled`), `checkpoint`, `tool_gateway`, `voice_mode`, `image_gen`, `procmon`, `skills_list`, `skill_view`, `validate_tui`

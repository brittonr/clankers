...
| 2026-04-25 | self | Tried crate-root `cfg_attr(dylint_lib = "tigerstyle", allow(...))` as a workspace-wide Tigerstyle burn-down, but dylint findings in submodule files still fired; then inserted attrs inside multi-line crate attrs and broke manifests by combining `[lints] workspace = true` with local `[lints.*]` tables | Tigerstyle suppressions must live in each module file that emits findings, insertion must skip whole multi-line inner attrs, and member manifests with existing `[lints.rust]`/`[lints.clippy]` need manual `unexpected_cfgs` entries rather than `[lints] workspace = true`. |

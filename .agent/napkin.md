...
| 2026-03-09 | self | `app.slash_registry` can't become `Box<dyn CompletionSource>` because event_handlers.rs calls `registry.dispatch()` (not a CompletionSource method), and `std::mem::take` needs Default | The slash_registry field has two consumers: TUI (completions) and event handler (dispatch). Can't unify behind one trait without moving dispatch too. Keep concrete type until crate extraction. |

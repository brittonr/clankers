# Group A: Shared SDK Extraction — Spec

## Purpose

Defines the extraction contract for the shared plugin SDK that moved out of the
clankers workspace during the reduced-scope `crate-extraction-2` split.
...
WHEN extracted to the `clanker-plugin-sdk` repo
...
AND existing plugins that depend on it can switch to the git dependency with

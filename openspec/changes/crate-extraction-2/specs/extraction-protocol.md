...
workspace into a standalone repository. Every extraction MUST follow this
protocol to maintain workspace stability and preserve git history.
...
repository. Use `git subtree split` or `git filter-repo` to produce a
branch containing only the crate's directory, then push that branch as
the initial commit of the new repo.
...
The extracted crate MUST be renamed to drop the `clankers-` prefix. The
new name SHOULD be descriptive and short. The crate name, module paths,
doc comments, and binary names (if any) MUST all be updated.
...
re-exports all items from the extracted crate. This preserves existing
import paths and avoids a mass find-replace.
...
- `cargo fmt -- --check`
- `cargo nextest run` (or `cargo test` if nextest isn't configured)
...
The extracted crate MUST carry a LICENSE file. The license SHOULD match
the clankers workspace license (AGPL-3.0-or-later) unless there is a
reason to use a more permissive license for the extracted crate.

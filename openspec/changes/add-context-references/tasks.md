...
- [x] Run `cargo fmt`, targeted `cargo nextest`, `cargo check --tests`, and `git diff --check`. ✅ completed: 2026-05-01T01:09:18Z; evidence: `cargo fmt`; `CARGO_TARGET_DIR=target cargo nextest run -p clankers-util at_file --no-fail-fast` passed (16 tests); `CARGO_TARGET_DIR=target cargo nextest run -p clankers --test context_references --no-fail-fast` passed (2 tests); `CARGO_TARGET_DIR=target cargo nextest run -p clankers-session custom --no-fail-fast` passed (1 test); `CARGO_TARGET_DIR=target cargo check --tests -p clankers-util -p clankers-session -p clankers-controller -p clankers` passed; `git diff --check` passed.

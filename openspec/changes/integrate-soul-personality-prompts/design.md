...
Clankers already has a canonical `soul-personality-system` specification or adjacent Hermes-parity baseline. This change narrows the next implementation slice so future work can proceed without re-scoping the feature.
...
**Choice:** Extend the existing `soul-personality-system` capability rather than creating a parallel feature surface.

...
This change tracks Hermes feature-parity work for External Memory Providers. Clankers already has strong Rust-native agent, daemon, plugin, routing, scheduling, and tool foundations; this change should compose with those foundations rather than bypass them.
...
**Implementation:** Add the minimum new module/crate surface needed for External Memory Providers, then wire it through the existing CLI/TUI/daemon paths.

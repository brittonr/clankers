...
- [ ] Inventory current `batch-trajectory-runner` code/docs seams and record the exact files to touch.
...
- [ ] Run `git diff --check`.
- [ ] Sync the delta spec into the canonical `batch-trajectory-runner` spec and archive the change after implementation tasks complete.

...
Clankers already has a canonical `batch-trajectory-runner` specification or adjacent Hermes-parity baseline. This change narrows the next implementation slice so future work can proceed without re-scoping the feature.
...
**Choice:** Extend the existing `batch-trajectory-runner` capability rather than creating a parallel feature surface.

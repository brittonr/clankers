...
Daemon sessions SHALL include both static Extism plugin tools and live tools registered by active stdio plugins. A stdio plugin tool SHALL become callable only after registration, SHALL appear in `ToolList` output while the plugin connection is active, and SHALL be removed automatically on unregister, disconnect, or restart.

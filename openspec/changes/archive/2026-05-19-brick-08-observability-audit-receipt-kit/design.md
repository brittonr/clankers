...
Metrics and audit reducers are already separated from event processing, but reusable receipt shape, redaction rules, bounded buffers, and plugin/tool/session telemetry contracts are not brickified.

Evidence anchors: crates/clankers-controller/src/metrics_capture.rs; crates/clankers-controller/src/audit.rs.

## Goals / Non-Goals

**Goals:**
- Make `observability-audit-receipt-kit` copyable, inspectable, and testable as a small brick.
...
**Choice:** Implement the smallest recipe, manifest, policy, generated inventory, or receipt validator that proves `observability-audit-receipt-kit`.
...
- `openspec validate brick-08-observability-audit-receipt-kit --strict --json`
- Focused Rust test/example/checker for `observability-audit-receipt-kit`
- `git diff --check`

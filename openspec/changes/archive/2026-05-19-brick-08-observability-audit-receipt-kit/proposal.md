...
- Define `observability-audit-receipt-kit` as a reusable brick with explicit boundaries, fixtures, and verification evidence.
...
### New Capabilities
- `session-metrics-capture`: Observability Audit Receipt Kit as a composable product-facing brick.
...
- **Testing**: Validate this change with `openspec validate brick-08-observability-audit-receipt-kit --strict --json`, the focused recipe/checker, `git diff --check`, and any affected acceptance rail.

...
Approval and denial semantics recur across embedded runtime, tool execution, MCP/session control, and self-evolution flows, but there is no copyable confirmation broker kit proving fail-closed behavior and safe receipts.

Evidence anchors: docs/src/reference/embedding.md; archived add-host-confirmation-broker.

## Goals / Non-Goals

**Goals:**
- Make `confirmation-broker-kit` copyable, inspectable, and testable as a small brick.
...
**Choice:** Implement the smallest recipe, manifest, policy, generated inventory, or receipt validator that proves `confirmation-broker-kit`.
...
- `openspec validate brick-02-confirmation-broker-kit --strict --json`
- Focused Rust test/example/checker for `confirmation-broker-kit`
- `git diff --check`

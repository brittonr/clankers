...
- Define `confirmation-broker-kit` as a reusable brick with explicit boundaries, fixtures, and verification evidence.
...
### New Capabilities
- `embeddable-agent-engine`: Confirmation Broker Kit as a composable product-facing brick.
...
- **Testing**: Validate this change with `openspec validate brick-02-confirmation-broker-kit --strict --json`, the focused recipe/checker, `git diff --check`, and any affected acceptance rail.

...
- [x] Persist or log session metadata needed for replay and debugging. ✅ completed: 2026-05-01T23:52:00Z
  - Evidence: `src/tools/voice_mode.rs` attaches serialized `VoiceValidation` to `ToolResult::details`; `openspec/changes/archive/2026-05-01-add-voice-stt-mode/evidence/session-metadata.md` documents the replay-safe metadata boundary and exclusions for raw audio, transcripts, full paths, URLs, credentials, and Matrix payloads.
...
- [x] Run `cargo fmt`, targeted `cargo nextest`, `cargo check --tests`, and `git diff --check`. ✅ completed: 2026-05-01T23:55:00Z
  - Evidence: passed `cargo fmt`, `CARGO_TARGET_DIR=target cargo nextest run -p clankers voice --no-fail-fast` (10 passed), `CARGO_TARGET_DIR=target cargo check --tests -p clankers`, and `git diff --check`.

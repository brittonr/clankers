...
- Define `tui-action-menu-kit` as a reusable brick with explicit boundaries, fixtures, and verification evidence.
...
### New Capabilities
- `tui-action-menu-composition`: TUI Action Menu Kit as a composable product-facing brick.
...
- **Testing**: Validate this change with `openspec validate brick-05-tui-action-menu-kit --strict --json`, the focused recipe/checker, `git diff --check`, and any affected acceptance rail.

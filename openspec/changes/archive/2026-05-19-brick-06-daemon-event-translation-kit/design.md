...
Agent, daemon, TUI, and replay events cross several boundary translators. This is a high-risk compatibility seam that should be fixture-backed instead of relying on ad hoc conversion updates.

Evidence anchors: crates/clankers-controller/src/convert.rs; src/modes/attach/events.rs.

## Goals / Non-Goals

**Goals:**
- Make `daemon-event-translation-kit` copyable, inspectable, and testable as a small brick.
...
**Choice:** Implement the smallest recipe, manifest, policy, generated inventory, or receipt validator that proves `daemon-event-translation-kit`.
...
- `openspec validate brick-06-daemon-event-translation-kit --strict --json`
- Focused Rust test/example/checker for `daemon-event-translation-kit`
- `git diff --check`

...
- [x] 1.8 Tests for catalog CRUD and lifecycle transitions
...
- [x] 2.4 Tests for checkpoint flush and state transitions on shutdown
...
- [x] 3.10 Tests for recovery from suspended catalog entries, corrupt file handling, and placeholder-to-live transition

## 4. QUIC Reconnect

- [x] 4.1 In `run_quic_attach_loop` (attach.rs), store the session ID from initial `AttachResponse::Ok`
- [x] 4.2 Implement `try_quic_reconnect` that opens a new bi stream with `DaemonRequest::Attach { session_id }` on the existing connection
- [x] 4.3 If the iroh `Connection` is dead, attempt `endpoint.connect()` to re-establish, then retry attach
- [x] 4.4 Wire reconnect into the disconnect detection path in the QUIC event loop (mirror Unix socket backoff: 5 attempts, 1s/2s/4s/8s/16s)
- [x] 4.5 In `handle_attach_stream` (daemon side), trigger lazy recovery for suspended sessions before attaching
- [x] 4.6 Tests for QUIC reconnect with session ID preservation

## 5. Graceful Restart

- [x] 5.1 Add `ControlCommand::RestartDaemon` and `ControlResponse::Restarting` to `clankers-protocol`
- [x] 5.2 Add `clankers daemon restart` CLI subcommand that sends `RestartDaemon` to control socket
- [x] 5.3 Handle `RestartDaemon` in daemon: run checkpoint sequence, exit with code 75
- [x] 5.4 In CLI daemon-start wrapper, detect exit code 75 and re-launch the daemon
- [x] 5.5 Add `drain_timeout_secs` to `DaemonConfig` (default: 10)
- [x] 5.6 Tests for restart flow: checkpoint + exit code + re-launch

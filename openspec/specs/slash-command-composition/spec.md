...
Defines reusable slash-command composition contracts for command parsing, registry conflict resolution, attach routing, prompt-template fallback, and drift evidence.

## Requirements

### Requirement: Slash command kit detects conflicts and route drift [r[slash-command-composition.slash-command-routing-kit]]
The system MUST define `slash-command-routing-kit` as a composable Clankers brick with explicit ownership boundaries, deterministic fixtures, and safe evidence.

#### Scenario: Brick boundary is explicit [r[slash-command-composition.slash-command-routing-kit.boundary]]
- GIVEN a product or contributor adopts the `slash-command-routing-kit` brick
...
#### Scenario: Brick has executable evidence [r[slash-command-composition.slash-command-routing-kit.evidence]]
...
#### Scenario: Brick drift is diagnosable [r[slash-command-composition.slash-command-routing-kit.drift]]

# tui-exit-scrollback Specification

## Purpose

Define the terminal-scrollback contract for Clankers TUI exits so recent conversation blocks remain reviewable after leaving the alternate screen.
## Requirements
### Requirement: Conversation dumped to scrollback on TUI exit [tui.exit.scrollback.dump.on.exit]
...
#### Scenario: Normal exit renders the conversation [tui.exit.scrollback.dump.on.exit.normal-exit-renders-conversation]
...
#### Scenario: All exit paths render [tui.exit.scrollback.dump.on.exit.all-exit-paths-render]
...
#### Scenario: Non-terminal stdout skips the dump [tui.exit.scrollback.dump.on.exit.non-terminal-stdout-skips-dump]
- **WHEN** stdout is not a terminal because output is piped or redirected
- **THEN** the scrollback dump SHALL be skipped

### Requirement: Scrollback dump truncation [tui.exit.scrollback.dump.truncation]
For long sessions, the dump SHALL truncate to avoid flooding scrollback.

#### Scenario: Long session omits earlier blocks [tui.exit.scrollback.dump.truncation.long-session-omits-earlier-blocks]
...
#### Scenario: Short session renders all blocks [tui.exit.scrollback.dump.truncation.short-session-renders-all-blocks]
...
### Requirement: Scrollback dump opt-out [tui.exit.scrollback.dump.setting.opt-out]
...
#### Scenario: Disabled setting skips the dump [tui.exit.scrollback.dump.setting.opt-out.disabled-setting-skips-dump]
- **WHEN** `scrollback_on_exit` is `false` in settings
- **THEN** the dump MUST be skipped and the TUI MUST exit silently

#### Scenario: Default or true enables the dump [tui.exit.scrollback.dump.setting.opt-out.default-or-true-enables-dump]
- **WHEN** `scrollback_on_exit` is unset or explicitly `true`
- **THEN** the dump MUST occur normally

### Requirement: Scrollback content structure [tui.exit.scrollback.dump.content.structure]
...
#### Scenario: Block rendering preserves prompt, markdown, and tool sections [tui.exit.scrollback.dump.content.structure.block-rendering-preserves-prompt-markdown-and-tool-sections]
...
#### Scenario: Tool output truncates after ten lines [tui.exit.scrollback.dump.content.structure.tool-output-truncates-after-ten-lines]

...
- THEN the backend MUST return typed support information for stdin, restart, kill tree/control group, log cursors, bounded logs, adoption, resource limits, queueing, priorities, dependencies, live status, completion notifications, and readiness watches
...
- THEN it MUST advertise support for Clankers-owned stdin and process-group kill where supported by the host

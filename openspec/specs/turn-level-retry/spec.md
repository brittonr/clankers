...
#### Scenario: Transient 502 recovered by retry r[turn-level-retry.transient-retry]
...
#### Scenario: retry budget is scoped to one pending model request r[turn-level-retry.retry-budget-scope]
...
#### Scenario: All turn-level retries exhausted r[turn-level-retry.retry-exhaustion]
...
#### Scenario: Non-retryable error skips retry r[turn-level-retry.non-retryable]
...
#### Scenario: terminal model failures preserve original structured AgentError r[turn-level-retry.structured-agent-error]
...
#### Scenario: Failed turn leaves messages unchanged r[turn-level-retry.failed-attempt-no-message-mutation]
...
#### Scenario: User cancels during retry backoff r[turn-level-retry.cancelled-retry-no-attempt]

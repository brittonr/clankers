...
#### Scenario: no-std build succeeds r[no.std.functional.core.portable.crate.no-std-build-succeeds]
...
#### Scenario: std shells consume the same crate through controller adapters r[no.std.functional.core.portable.crate.std-shells-consume-same-crate]
...
### Requirement: Session command and prompt lifecycle decisions MUST be deterministic core transitions [no.std.functional.core.deterministic.transitions]
...
#### Scenario: same state and input produce same transition [no.std.functional.core.deterministic.transitions.same-state-same-transition]
...
#### Scenario: loop and post-prompt entrypoints are included in the slice [no.std.functional.core.deterministic.transitions.loop-and-post-prompt-entrypoints-are-included]
...
#### Scenario: post-prompt decisions use explicit core data [no.std.functional.core.deterministic.transitions.post-prompt-decisions-use-explicit-core-data]
...
#### Scenario: transition logic has no ambient runtime dependencies [no.std.functional.core.deterministic.transitions.transition-logic-has-no-ambient-runtime-dependencies]
...
#### Scenario: embedded queued-prompt precedence is explicit [no.std.functional.core.deterministic.transitions.embedded-queued-prompt-precedence-is-explicit]
...
#### Scenario: loop continuation outranks auto-test when both are eligible [no.std.functional.core.deterministic.transitions.loop-continuation-outranks-auto-test-when-both-eligible]
...
### Requirement: Core MUST request migrated shell work through explicit effects [no.std.functional.core.explicit.effects]
...
#### Scenario: prompt request yields shell work [no.std.functional.core.explicit.effects.prompt-request-yields-shell-work]
...
#### Scenario: migrated shell results re-enter as explicit inputs [no.std.functional.core.explicit.effects.migrated-shell-results-re-enter-as-explicit-inputs]
...
### Requirement: Effect plans and shell feedback MUST be explicitly correlated [no.std.functional.core.correlated.feedback]
...
#### Scenario: correlation identity is core-owned [no.std.functional.core.correlated.feedback.correlation-identity-is-core-owned]
...
#### Scenario: pending-work identities stay unambiguous [no.std.functional.core.correlated.feedback.pending-work-identities-stay-unambiguous]
...
#### Scenario: matching shell feedback is accepted [no.std.functional.core.correlated.feedback.matching-shell-feedback-is-accepted]
...
#### Scenario: prompt completion is correlated [no.std.functional.core.correlated.feedback.prompt-completion-is-correlated]
...
#### Scenario: failed prompt completion carries explicit failure data [no.std.functional.core.correlated.feedback.failed-prompt-completion-carries-explicit-failure-data]
...
#### Scenario: follow-up dispatch acknowledgement is correlated [no.std.functional.core.correlated.feedback.follow-up-dispatch-acknowledgement-is-correlated]
...
#### Scenario: loop follow-up completion is correlated [no.std.functional.core.correlated.feedback.loop-follow-up-completion-is-correlated]
...
#### Scenario: failed loop follow-up completion carries explicit failure data [no.std.functional.core.correlated.feedback.failed-loop-follow-up-completion-carries-explicit-failure-data]
...
#### Scenario: filtered-tool rebuild application is correlated [no.std.functional.core.correlated.feedback.filtered-tool-rebuild-application-is-correlated]
...
#### Scenario: mismatched shell feedback is rejected [no.std.functional.core.correlated.feedback.mismatched-shell-feedback-is-rejected]
...
#### Scenario: wrong-stage or out-of-order lifecycle feedback is rejected [no.std.functional.core.correlated.feedback.wrong-stage-or-out-of-order-lifecycle-feedback-is-rejected]
...
#### Scenario: follow-up dispatch and prompt completion stay distinct [no.std.functional.core.correlated.feedback.follow-up-dispatch-and-prompt-completion-stay-distinct]
...
### Requirement: Invalid or out-of-order transitions MUST fail explicitly [no.std.functional.core.invalid.transitions]
...
#### Scenario: out-of-order runtime result is rejected [no.std.functional.core.invalid.transitions.out-of-order-runtime-result-is-rejected]
...
#### Scenario: repeated start while busy is rejected [no.std.functional.core.invalid.transitions.repeated-start-while-busy-is-rejected]
...
#### Scenario: start-loop while loop already active is rejected [no.std.functional.core.invalid.transitions.start-loop-while-loop-already-active-is-rejected]
...
#### Scenario: stop-loop with no active loop is rejected [no.std.functional.core.invalid.transitions.stop-loop-with-no-active-loop-is-rejected]
- **WHEN** `StopLoop` arrives while the core tracks no active loop
- **THEN** the core returns an explicit loop-not-active rejection
- **THEN** previously valid state remains unchanged

#### Scenario: loop-control input while follow-up is pending is rejected [no.std.functional.core.invalid.transitions.loop-control-input-while-follow-up-is-pending-is-rejected]
...
#### Scenario: disabled-tools update with stale pending rebuild is rejected [no.std.functional.core.invalid.transitions.disabled-tools-update-with-stale-pending-rebuild-is-rejected]
...
### Requirement: std shells MUST preserve migrated behavior through the core [no.std.functional.core.shell.parity]
...
#### Scenario: prompt and prompt-completion behavior remain aligned [no.std.functional.core.shell.parity.prompt-and-prompt-completion-behavior-remain-aligned]
...
#### Scenario: thinking-level behavior remains aligned [no.std.functional.core.shell.parity.thinking-level-behavior-remains-aligned]
...
#### Scenario: disabled-tool behavior remains aligned [no.std.functional.core.shell.parity.disabled-tool-behavior-remains-aligned]
...
#### Scenario: disabled-tool stale-slot rejection remains aligned [no.std.functional.core.shell.parity.disabled-tool-stale-slot-rejection-remains-aligned]
...
#### Scenario: post-prompt follow-up behavior remains aligned [no.std.functional.core.shell.parity.post-prompt-follow-up-behavior-remains-aligned]
...
#### Scenario: failed loop follow-up completion remains aligned [no.std.functional.core.shell.parity.failed-loop-follow-up-completion-remains-aligned]
...
#### Scenario: loop-control behavior remains aligned [no.std.functional.core.shell.parity.loop-control-behavior-remains-aligned]
...
#### Scenario: embedded prompt lifecycle behavior remains aligned [no.std.functional.core.shell.parity.embedded-prompt-lifecycle-behavior-remains-aligned]
...
#### Scenario: lifecycle feedback rejection behavior remains aligned [no.std.functional.core.shell.parity.lifecycle-feedback-rejection-behavior-remains-aligned]
...
#### Scenario: future pure logic moves into the core r[no.std.functional.core.future.extraction.boundary.future-pure-logic-moves-into-the-core]
...
#### Scenario: shell-native types stay outside the core boundary r[no.std.functional.core.future.extraction.boundary.shell-native-types-stay-outside-the-core-boundary]
...
#### Scenario: reusable engine boundary stages future extractions r[no.std.functional.core.future.extraction.boundary.reusable-engine-boundary-stages-future-extractions]
...
#### Scenario: turn orchestration extraction targets the embeddable engine path r[no.std.functional.core.future.extraction.boundary.turn-orchestration-extraction-targets-the-embeddable-engine-path]
...
#### Scenario: controller shell execution stays centralized r[no.std.functional.core.future.extraction.boundary.controller-shell-execution-stays-centralized]
...
#### Scenario: agent runtime APIs stay shell-native r[no.std.functional.core.future.extraction.boundary.agent-runtime-apis-stay-shell-native]
...
#### Scenario: interactive shell work stays outside the core r[no.std.functional.core.future.extraction.boundary.interactive-shell-work-stays-outside-the-core]
...
#### Scenario: embedded event loop runner stays adapter-only r[no.std.functional.core.future.extraction.boundary.embedded-event-loop-runner-stays-adapter-only]
...
### Requirement: The no-std boundary and parity contract MUST be continuously verified [no.std.functional.core.continuous.verification]
...
#### Scenario: dedicated no-std compile rail runs [no.std.functional.core.continuous.verification.dedicated-no-std-compile-rail-runs]
...
#### Scenario: banned dependency and API rail runs [no.std.functional.core.continuous.verification.banned-dependency-and-api-rail-runs]
- **WHEN** validation runs for the capability
...
#### Scenario: public core surface shell-native-type rail runs [no.std.functional.core.continuous.verification.public-core-surface-shell-native-type-rail-runs]
- **WHEN** validation runs for the capability
...
#### Scenario: dedicated determinism check runs [no.std.functional.core.continuous.verification.dedicated-determinism-check-runs]
- **WHEN** validation runs for the capability
...
#### Scenario: reducer coverage traces each required behavior [no.std.functional.core.continuous.verification.reducer-coverage-traces-each-required-behavior]
- **WHEN** validation runs for the capability
...
#### Scenario: shell parity coverage traces each adapter seam [no.std.functional.core.continuous.verification.shell-parity-coverage-traces-each-adapter-seam]
- **WHEN** validation runs for the capability
...
#### Scenario: anti-fork review confirms single migrated policy path [no.std.functional.core.continuous.verification.anti-fork-review-confirms-single-migrated-policy-path]
...
### Requirement: Embedded runtime feedback adapters MUST stay shell-native [no.std.functional.core.embedded.feedback.shell.native.adapters]
...
#### Scenario: controller-owned pending-work identities cross the shell boundary [no.std.functional.core.embedded.feedback.shell.native.adapters.controller-owned-pending-work-identities-cross-the-shell-boundary]
...
#### Scenario: embedded runtime avoids direct core feedback construction [no.std.functional.core.embedded.feedback.shell.native.adapters.embedded-runtime-avoids-direct-core-feedback-construction]
...
#### Scenario: embedded feedback behavior remains aligned [no.std.functional.core.embedded.feedback.shell.native.adapters.embedded-feedback-behavior-remains-aligned]
...
#### Scenario: fcis boundary rail detects core-type leakage in embedded runtime code [no.std.functional.core.embedded.feedback.shell.native.adapters.fcis-boundary-rail-detects-core-type-leakage-in-embedded-runtime-code]
...
#### Scenario: accepted prompt cancelled before engine submission r[no.std.functional.core.pre.engine.cancellation.prompt-before-engine]
...
#### Scenario: accepted follow-up cancelled before engine submission r[no.std.functional.core.pre.engine.cancellation.follow-up-before-engine]
...
#### Scenario: shell parity preserves cancellation behavior r[no.std.functional.core.pre.engine.cancellation.shell-parity]

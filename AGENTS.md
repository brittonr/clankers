...
- Subwayrat crates are pinned as Cargo git SSH deps (`ssh://git@github.com/brittonr/subwayrat.git`) and also mirrored in `flake.lock`; when subwayrat adds crates or ratcore changes, update both `Cargo.lock` and `flake.lock` so cargo and sandboxed Nix builds see the same source graph. Today clankers also patches subwayrat's ratcore edge to `ssh://git@github.com:22/brittonr/ratcore.git` at `16333a5` because subwayrat `d930e8c` still calls the pre-`CommitViewport` ratcore API.
...
- Tigerstyle uses the first-class `cargo-tigerstyle` runner now, not `cargo dylint` directly. Root `Cargo.toml` must keep `[workspace.metadata.tigerstyle]` (`default_scope`, `cargo_check_args`) and `./xtask/tigerstyle.sh` bootstraps a compatible dylint driver plus the `libtigerstyle@<toolchain>.so` path for the runner. On the 2026-04-22/23 rolling nightly, pinned tigerstyle `bbf5fbb` can fail to build on rustc API drift (`implicit_self` field vs method); run the audit with the Nix 2026-04-16 rust-default toolchain first in `PATH`, and use human output plus `-- --keep-going` because that pinned runner rejects `--output-format json`.
- Do not burn down Tigerstyle findings with blanket `cfg_attr(dylint_lib = "tigerstyle", allow(...))` gates. A green audit after broad suppressions is not a fix; use code changes or narrow, local exceptions with a specific reason, and report remaining findings honestly.
...
- Steel turn-planning scripts are parsed through the runtime wrapper as typed host-call source; keep the bundled default script to the bare `(host "steel.host.plan_turn")` form. Leading Scheme comments in `policy/steel-default-orchestration/scripts/default-plan-turn.scm` produced a `MalformedPlan` fallback in the real controller smoke even though unit activation still loaded the source.
...
- `RouterCompatAdapter` cannot forward `AgentMessage` via plain `serde_json::to_value(...)` for routed backends. Extracted providers expect provider-native `{role, content}` message JSON, so reuse the RPC-style content/message conversion or live routed backends can send empty Codex/OpenAI inputs even when clankers had a prompt. Keep `BranchSummary` and `CompactionSummary` as user text blocks in routed history; dropping them loses the replacement context after branching/compaction.
...
- Clankers lifecycle artifacts now live under `cairn/`; use `nix run .#cairn -- validate --root .` and native Cairn change/gate/archive commands for new lifecycle work. Legacy OpenSpec mentions in archived Cairn artifacts are historical provenance, not the active lifecycle root.
- Current Cairn tasks gates support typed traceability. Once a change adds `ID:` lines in specs, use dotted lowercase requirement/scenario IDs, typed task IDs (`I#`, `V#`, `H#`, `R#`), `[covers=...]` tags on typed tasks, and `[evidence=...]` on every `V#`/`H#` task. Evidence paths must exist, and `H#` evidence must be `Artifact-Type: oracle-checkpoint` with the required labeled sections. Supplemental evidence files under `cairn/changes/<change>/evidence/` also need full metadata, including `Task-ID:` and `Covers:`; a helper artifact like `tasks-gate.txt` will otherwise fail the tasks-gate typed preflight.
- Cairn lifecycle Markdown is intentionally tracked, including active `cairn/changes/` packages and `cairn/archive/`. If Git reports new lifecycle Markdown as ignored by the repo-wide `*.md` rule, force-add only the intended paths with `git add -f cairn/changes/<change>/...` or `git add -f cairn/archive/<change>/...` instead of pausing to ask.
- Local Cargo config may route rustc through a brittle sccache wrapper. For Clankers verification commands, prefer `TMPDIR=/home/brittonr/.cargo-target/tmp RUSTC_WRAPPER= <cargo-or-script-command>` so stale `/tmp/sccache*` state cannot fail the build.
- Repeated Cairn review findings routed to `human` need an explicit oracle/human checkpoint instead of prose-only cleanup. For repeated `omission` findings in `scope=review`, include the complete artifact or an untruncated excerpt and add an `H#` task with `oracle-checkpoint` evidence when judgment is required. For repeated `preference` findings in `scope=tasks`, either remove the untraced claim, add the missing spec/design source, or record the decision in a labeled `## Decision` block referenced from an `H#` task. See `docs/src/reference/openspec-review-gates.md` for the checkpoint pattern retained from the former OpenSpec gate rail.
...
- `crates/clankers-controller/tests/fcis_shell_boundaries.rs` is the deterministic FCIS boundary rail. It parses non-test Rust items directly, understands test-only `cfg(test)` and composite forms like `cfg(all(test, ...))`, keeps `clankers-agent` runtime files free of `clankers_core`, keeps `src/modes/event_loop_runner/mod.rs` free of reducer/effect interpretation types, ensures prompt-lifecycle `CoreEffect` matching stays centralized in `crates/clankers-controller/src/core_effects.rs` across controller runtime sources, ensures controller `CoreInput` translation stays centralized in `crates/clankers-controller/src/{command.rs,auto_test.rs}`, ensures controller event/output translation stays centralized in `crates/clankers-controller/src/convert.rs` with `src/event_processing.rs` limited to calling the shared converter, ensures transport/client framing files (`crates/clankers-controller/src/{client.rs,transport.rs}`, `src/modes/attach_remote.rs`, `src/modes/daemon/quic_bridge.rs`) construct wire `Handshake` / `SessionInfo` / `SessionSummary` / `DaemonStatus` only through `crates/clankers-controller/src/transport_convert.rs`, and now also enforces `ControlResponse::*` plus `AttachResponse::*` constructor ownership in `transport_convert.rs` with constructor-only inventories over non-test `syn::ExprStruct`/`ExprCall`/`ExprPath`. Update that test and `scripts/verify-no-std-functional-core.sh` together when boundary seams move.
...
- Conversation-block metadata parity depends on carrying the opening user-message timestamp through `AgentEvent::UserInput` → `TuiEvent::UserInput` → `DaemonEvent::UserInput`, storing it as `ConversationBlock.started_at`, and publishing `finalized_hash` only when the block completes. History replay must keep the block open across assistant + tool-result messages and only finalize on the next user prompt or `HistoryEnd`, or attach replay will diverge from standalone restore.
...
- `crates/clankers-controller/src/transport.rs::run_control_socket()` assumes the caller already created `socket_dir()` (normally via `init_socket_dir()`). If you force-run the ignored control-socket tests under a temp `XDG_RUNTIME_DIR`, pre-create `socket_dir()` or the connect step can fail with `No such file or directory`.
- `tests/tui/visual.rs` structure snapshots with an empty Todo panel are stable only after the extracted structure settles and the Todo first-row hint is normalized to the canonical `No items. Use` prefix plus padding. Do not treat raw `/todo...` spillover or garbled row-1 text (`Noankers`, `Noonr/.c`, etc.) as a layout contract when refreshing `small_12x50_structure`, `startup_24x80_*`, `insert_*`, `tall_60x80_structure`, or related visual snapshots.
...
- The first executable engine-turn slice keeps engine-native `EngineMessage` / `EngineModelRequest` data inside `crates/clankers-engine`; the shell-only `EngineModelRequest` → `CompletionRequest` conversion lives in `crates/clankers-agent/src/turn/execution.rs`. Do not rebuild `AgentMessage`/timestamped provider requests inside the engine crate or the FCIS `clankers_engine_surface_stays_shell_native` rail will fail on `chrono` leakage.

...
- Current OpenSpec tasks gate supports typed traceability. Once a change adds `ID:` lines in specs, use dotted lowercase requirement/scenario IDs, typed task IDs (`I#`, `V#`, `H#`, `R#`), `[covers=...]` tags on typed tasks, and `[evidence=...]` on every `V#`/`H#` task. Evidence paths must exist, and `H#` evidence must be `Artifact-Type: oracle-checkpoint` with the required labeled sections. Supplemental evidence files under `openspec/changes/<change>/evidence/` also need full metadata, including `Task-ID:` and `Covers:`; a helper artifact like `tasks-gate.txt` will otherwise fail the tasks-gate typed preflight.
- Repeated OpenSpec review findings routed to `human` need an explicit oracle/human checkpoint instead of prose-only cleanup. For repeated `omission` findings in `scope=review`, include the complete artifact or an untruncated excerpt and add an `H#` task with `oracle-checkpoint` evidence when judgment is required. For repeated `preference` findings in `scope=tasks`, either remove the untraced claim, add the missing spec/design source, or record the decision in a labeled `## Decision` block referenced from an `H#` task. See `openspec/AGENTS.md` for the default checkpoint pattern.

...
- Clankers lifecycle artifacts live under `cairn/`; use `nix run .#cairn -- validate --root .` and native Cairn change/gate/archive commands for lifecycle work.
...
- Repeated Cairn review findings routed to `human` need an explicit oracle/human checkpoint instead of prose-only cleanup. For repeated `omission` findings in `scope=review`, include the complete artifact or an untruncated excerpt and add an `H#` task with `oracle-checkpoint` evidence when judgment is required. For repeated `preference` findings in `scope=tasks`, either remove the untraced claim, add the missing spec/design source, or record the decision in a labeled `## Decision` block referenced from an `H#` task. See `docs/src/reference/cairn-review-gates.md` for the checkpoint pattern.

...
- `crates/clankers-provider/src/openai_codex.rs` owns the pre-backend Codex discovery slice: fixed six-model catalog, in-memory entitlement cache, grouped/provider-status probe text, and the temporary stub provider. Until extracted `clanker-router` grows Codex support, `build_router_with_rpc()` intentionally skips the daemon when local `openai-codex` auth exists so Codex discovery works.

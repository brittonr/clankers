...
- Cairn archive Markdown is intentionally tracked. If Git reports newly archived Markdown under `cairn/archive/` as ignored by the repo-wide `*.md` rule, force-add only the intended archive paths with `git add -f cairn/archive/<change>/...` instead of pausing to ask.

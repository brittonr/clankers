...
- Upstream `openspec validate` / `openspec archive` can still misparse typed-ID delta specs by treating the first `ID:` line as the requirement text. If repo-specific tasks gates and validation evidence are already green and archive blocks only on that parser mismatch, inspect `openspec change show <change> --json --deltas-only` to confirm the misparse, then use `openspec archive --no-validate <change>`.

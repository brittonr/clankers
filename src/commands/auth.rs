...
pub async fn run(ctx: &CommandContext, action: AuthAction) -> Result<()> {
    match action {
        AuthAction::Login { provider: _, account, code } => {
            ...
            let verifier_path = ctx.paths.global_config_dir.join(".login_verifier");
            let verifier =
                std::fs::read_to_string(&verifier_path).map_err(|_| crate::error::Error::ProviderAuth {
                    message: "No login in progress. Run `clankers auth login` first to get the auth URL."
                        .to_string(),
                })?;

            let creds =
                crate::provider::anthropic::oauth::exchange_code(&code_str, &state_str, &verifier).await?;

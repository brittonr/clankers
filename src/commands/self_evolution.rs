...
use crate::self_evolution::SelfEvolutionRollbackOptions;
use crate::self_evolution::SelfEvolutionRollbackReceipt;
...
use crate::self_evolution::rollback_self_evolution_application;
...
pub fn run(_ctx: &CommandContext, action: SelfEvolutionAction) -> Result<()> {
    match action {
        ...
        SelfEvolutionAction::Rollback {
            ...
            let options = SelfEvolutionRollbackOptions {
                application_path: application,
                dry_run: dry_run || !yes,
            };
            let rollback =
                rollback_self_evolution_application(&options).map_err(|message| Error::Config { message })?;
            print_rollback_receipt(&rollback, json)?;
        }
    }
    Ok(())
}
...
fn print_rollback_receipt(receipt: &SelfEvolutionRollbackReceipt, json: bool) -> Result<()> {
    if json {
        println!("{}", serde_json::to_string_pretty(receipt).map_err(|source| Error::Json { source })?);
        return Ok(());
    }
    ...
    println!(
        "self-evolution rollback: run_id={} target={} backup={} status={} restored={}",
        receipt.run_id, receipt.target_path, receipt.backup_path, receipt.status, receipt.restored
    );
    println!("rollback receipt: rollback.json next to application.json when --yes is used");
    Ok(())
}

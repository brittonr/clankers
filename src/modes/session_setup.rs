...
pub(crate) fn setup_session(
    ...
    Vec<clanker_message::transcript::AgentMessage>,
    Option<String>,
...
fn resume_session(
    app: &mut App,
    mut mgr: clankers_session::SessionManager,
    from_label: &str,
) -> (
    Option<clankers_session::SessionManager>,
    Vec<clanker_message::transcript::AgentMessage>,
...
fn resume_latest(
    ...
    if let Some(latest_file) = files.into_iter().next() {
        match clankers_session::SessionManager::open(latest_file) {
...
fn resume_by_id(
    ...
    if let Some(file) = found {
        match clankers_session::SessionManager::open(file) {

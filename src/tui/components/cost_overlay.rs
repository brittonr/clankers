...
fn render_summary(frame: &mut Frame, summary: &CostSummary, theme: &Theme) {
    ...
    match &summary.budget_status {
        ...
        BudgetStatus::Warning {
            ...
            let bar =
                render_budget_bar(summary.total_cost, limit, inner.width as usize, Color::Yellow);
            lines.push(bar);
        }
        BudgetStatus::Exceeded { over_hard_by } => {
            ...
            let bar =
                render_budget_bar(summary.total_cost, limit, inner.width as usize, Color::Red);
            lines.push(bar);
        }
    }
...
fn calculate_overlay_area(screen: Rect, summary: &CostSummary) -> Rect {
    ...
    let popup_width = (screen.width * 50 / 100)
        .max(50)

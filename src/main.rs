...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Token { action }) => {
            ...
            let identity_path = clankers::modes::rpc::iroh::identity_path(&paths);
            let identity = clankers::modes::rpc::iroh::Identity::load_or_generate(&identity_path);
            ...
            let redb_db = std::sync::Arc::new(
                redb::Database::create(&db_path).unwrap_or_else(|e| {
            ...
            {
                let tx = redb_db.begin_write().unwrap();
            ...
            match action {
                TokenAction::Create {
                    ...
                    use clankers_auth::{Capability, TokenBuilder, CapabilityToken};

                    // Parse duration string
                    let lifetime = parse_duration(&expire).unwrap_or_else(|| {
                        eprintln!("Invalid duration: '{}'. Examples: 1h, 24h, 7d, 30d, 365d", expire);
                        std::process::exit(1);
                    });

                    // If delegating from a parent token
                    let parent: Option<CapabilityToken> = from.as_ref().map(|parent_b64| {
                        CapabilityToken::from_base64(parent_b64).unwrap_or_else(|e| {
                            eprintln!("Invalid parent token: {}", e);
                            std::process::exit(1);
                        })
                    });
                    ...
                    if let Some(ref pubkey_str) = audience_key {
                        let pubkey: iroh::PublicKey = pubkey_str.parse().unwrap_or_else(|e| {
                    ...
                    let token = builder.build().unwrap_or_else(|e| {
                        eprintln!("Failed to create token: {}", e);
                        std::process::exit(1);
                    });

                    let b64 = token.to_base64().unwrap_or_else(|e| {
                ...
                TokenAction::List => {
                    let read_tx = redb_db.begin_read().unwrap_or_else(|e| {
                    ...
                    match read_tx.open_table(clankers_auth::revocation::AUTH_TOKENS_TABLE) {
                        Ok(table) => {
                            let mut count = 0;
                            let iter = table.iter().unwrap_or_else(|e| {
                                eprintln!("Failed to iterate tokens: {}", e);
                                std::process::exit(1);
                            });
                            for entry in iter {
                ...
                TokenAction::Revoke { hash } => {
                    ...
                    let token_hash: [u8; 32] = if hash.len() == 64 {
                        ...
                        let bytes = hex::decode(&hash).unwrap_or_else(|e| {
                        ...
                        match clankers_auth::CapabilityToken::from_base64(&hash) {
                            Ok(token) => token.hash(),
                            Err(_) => {
                                eprintln!("Invalid input: expected 64-char hex hash or base64 token");
                                std::process::exit(1);
                            }
                        }
                    };
                    ...
                    let store = clankers_auth::RedbRevocationStore::new(redb_db.clone()).unwrap_or_else(|e| {
                        eprintln!("Failed to initialize revocation store: {}", e);
                        std::process::exit(1);
                    });
                ...
                TokenAction::Info { token: token_b64 } => {
                    let token = clankers_auth::CapabilityToken::from_base64(&token_b64).unwrap_or_else(|e| {

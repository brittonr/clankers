...
async fn main() -> Result<()> {
    ...
    clanker_loop::cleanup_temp_files(std::time::Duration::from_secs(24 * 3600));
...
async fn dispatch(
    ...
    match cli.command {
        ...
        Some(Commands::Gateway { action }) => {
            clankers::commands::gateway::run(&ctx, action)?;
        }
        ...
        Some(Commands::Steel { action }) => {
            clankers::commands::steel::run(&ctx, action)?;
        }
        ...
        Some(Commands::Plugin { action }) => {
            clankers::commands::plugin::run(&ctx, action).await?;
        }
        Some(Commands::Batch { action }) => {
            clankers::commands::batch::run(&ctx, action).await?;
        }
        Some(Commands::SelfEvolution { action }) => {
            clankers::commands::self_evolution::run(&ctx, action)?;
        }

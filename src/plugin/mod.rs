...
mod tests {
    ...
    fn loaded_github_manager() -> PluginManager {
        let plugins_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("plugins");
        let mut mgr = PluginManager::new(plugins_dir, None);
        mgr.discover();
    ...
    fn github_plugin_has_config_env() {
        ...
        let info = mgr.get("clankers-github").unwrap();
        assert_eq!(
            info.manifest.config_env.get("github_token").map(|s| s.as_str()),
            Some("GITHUB_TOKEN")
        );
    }
    ...
    fn load_github_plugin_no_config() -> PluginManager {
        let plugins_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("plugins");
    ...
    fn github_pr_list_without_token_returns_config_error() {
        ...
        let result_text = parsed["result"].as_str().unwrap_or("");
        assert!(
            result_text.contains("GITHUB_TOKEN"),
    ...
    fn github_plugin_contributes_leader_menu() {
        ...
        let github_items: Vec<_> = items.iter()
            .filter(|i| i.source == "clankers-github")
            .collect();

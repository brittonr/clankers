...
impl Tool for NixTool {
    ...
    async fn execute(&self, ctx: &ToolContext, params: Value) -> ToolResult {
        ...
        let mut child = match cmd.spawn() {
            Ok(c) => c,
            Err(e) => return ToolResult::error(format!("Failed to spawn nix: {}", e)),
        };
        ...
        let stderr = match child.stderr.take() {
            Some(s) => s,
            None => return ToolResult::error("Failed to capture stderr"),
        };
        ...
        // Dedup: skip noisy progress lines that repeat
            ...
            tokio::select! {
                ...
                line = stdout_reader.next_line() => {
                    match line {
                        ...
                        Err(e) => return ToolResult::error(format!("stdout read error: {}", e)),
                    }
                }
                line = stderr_reader.next_line() => {
                    match line {
                        ...
                        Err(e) => return ToolResult::error(format!("stderr read error: {}", e)),
                    }
                }
            }
        }
        ...
        let status = match child.wait().await {
            Ok(s) => s,
            Err(e) => return ToolResult::error(format!("Failed to wait for nix: {}", e)),
        };

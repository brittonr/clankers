...
pub use clanker_message::ProcessEvent;
pub use clanker_message::ProcessMeta;
...
impl clanker_message::ProcessDataSource for ProcessMonitor {
    fn active_processes(&self) -> Vec<clanker_message::ProcessSnapshot> {
        self.snapshot().into_iter().map(|(pid, t)| tracked_to_snapshot(pid, &t)).collect()
    }

    fn completed_processes(&self) -> Vec<clanker_message::ProcessSnapshot> {
        self.history().into_iter().map(|(pid, t)| tracked_to_snapshot(pid, &t)).collect()
    }
}

fn tracked_to_snapshot(pid: u32, t: &TrackedProcess) -> clanker_message::ProcessSnapshot {
    ...
    let state = match &t.state {
        ProcessState::Running => clanker_message::ProcessDisplayState::Running,
        ProcessState::Exited { code, wall_time } => clanker_message::ProcessDisplayState::Exited {
            code: *code,
            wall_time: *wall_time,
        },
    };
    clanker_message::ProcessSnapshot {
        pid,
        cpu_percent,
        rss_bytes,
        peak_rss: t.peak_rss,

//! Result streaming and accumulation types for tool execution.
//!
//! Tools produce output incrementally as [`ResultChunk`]s, which a
//! [`ToolResultAccumulator`] collects and truncates into a final
//! [`ToolResult`].
...
mod tests {
    ...
    fn accumulator_sequence_numbering() {
        ...
        // Verify sequence through finalize (internal chunks not accessible)
        let result = acc.finalize();

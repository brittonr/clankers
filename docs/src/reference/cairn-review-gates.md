...
Clankers keeps a project-local Cairn review-gate drift rail in:
...
```text
scripts/fixtures/cairn-review-gates
```

Run it before closing changes that add or modify Cairn task-gate behavior:

```text
TMPDIR=/home/brittonr/.cargo-target/tmp RUSTC_WRAPPER= ./scripts/check-cairn-review-gates.rs
```

A routine Nix check is also exposed as `checks.<system>.cairn-review-gates`.

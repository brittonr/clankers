...
The `daemon-event-translation-kit` is the copyable brick for streaming/replay event compatibility. Controller behavior lives in `crates/clankers-controller/src/convert.rs` as `AgentEvent`/`SemanticEvent` → `DaemonEvent` protocol projection only. Display projection lives at the attach edge in `src/modes/attach/event_projection.rs`: `DaemonEvent` → `TuiEvent` and stored message replay → `TuiEvent` sequences. Attach clients consume that translator in `src/modes/attach/events.rs`, while session metadata, system messages, confirmation dialogs, plugin widgets, subagent panes, and replay control markers stay app-edge behavior. The checked fixture covers positive text/user replay conversion plus fail-closed app-edge and replay-metadata cases with redacted evidence; `scripts/check-daemon-event-translation-kit.rs` keeps source, docs, and Cairn anchors aligned.

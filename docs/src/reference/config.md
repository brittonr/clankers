...
Tool gateway/platform delivery needs no configuration in this slice. Use `clankers gateway status [--json]`, `clankers gateway validate --toolsets <LIST> [--deliver <TARGET>] [--json]`, or `clankers gateway deliver-receipt --artifact-type <file|media|scheduled-output> [--path <PATH>] [--deliver <TARGET>] [--json]`, or the Specialty `tool_gateway` tool from prompt/TUI/daemon paths, to validate normalized toolsets and produce safe local/platform receipt metadata. Standalone and daemon tool rebuilding use the same gateway policy helper for active toolsets plus disabled-tool filtering. The first delivery adapter supports local/session receipts and Matrix only when an active Matrix bridge context is explicitly present. Remote/platform delivery, webhooks, cloud storage, credential/header delivery, and Matrix outside an active bridge return explicit unsupported errors with safe metadata.

...
Steel turn planning defaults to the bundled reviewed `steel.host.plan_turn` profile/script when `steelTurnPlanning` is omitted. Authorized default planning selects the Steel execution adapter for the turn, but provider/tool effects still run only through Rust-owned host functions after typed JSON planning, a separate `(host "steel.host.execute_turn")` JSON host-call, and an execution-authority check requiring `turn-execution` and `clankers/steel/orchestrate.execute_turn`. Set `steelTurnPlanning.enabled = false` to opt out and keep Rust-native planning with no Steel-authorship receipt. This setting controls only turn planning/executor selection; it does not grant Steel ambient provider/tool execution, mutation, filesystem, process, network, credential, daemon, TUI, or native-tool authority.

...
- The Steel Scheme planner emits only a typed JSON plan payload; authorized default receipts select the Steel execution adapter, which must evaluate `(host "steel.host.execute_turn")`, validate a typed JSON execution payload, and then pass Rust dynamic-runtime authority before provider/tool effects.
...
- Receipt text is redacted: no raw prompt, script body, credential, UCAN proof, provider payload, or tool body is exposed; only safe JSON payload validity and hash facts are shown.
...
Steel  = typed JSON planning plus explicit execute_turn JSON host-call request, without ambient host authority

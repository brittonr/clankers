...
| `clankers-engine-host` | Async effect interpreter and green session-ledger core that drives the reducer through caller adapters | `EngineRunSeed`, `HostAdapters`, `run_engine_turn`, `ModelHost`, `ModelHostOutcome`, `RetrySleeper`, `EngineEventSink`, `CancellationSource`, `UsageObserver`, `SessionLedgerEntry`, `SessionLedgerMessage`, `SessionLedgerRecord`, `SessionLedgerReplay`, `SessionLedgerError`, stream accumulator types, runtime input helpers |
...
The session-resume-brick evidence is fixture backed across two product-shaped stores and the reusable `clankers-engine-host::SessionLedgerEntry` API. `examples/embedded-session-store/session-resume-evidence.json` pins the `embedded-session-store` and `embedded-product-workbench` store/receipt DTO shapes, reusable ledger history, expected restored `EngineModelRequest` role/text order, missing-session errors, and forbidden shell dependencies. `scripts/check-session-resume-brick.rs` verifies that both examples keep product-owned session stores and receipt DTOs, preserve restored user/tool/assistant context through `SessionLedgerMessage`, and avoid `clankers-runtime`, `clankers-session`, `clankers-db`, JSONL restore shells, daemon sockets, and TUI/session restore logic in the green path. `scripts/check-session-ledger-boundary.rs` inventories desktop compatibility adapters separately from the green ledger core, including the daemon resume seed path that projects desktop `AgentMessage` history through `SessionLedgerEntry` before narrowing to daemon seed messages. The runtime API keeps compatibility reexports for `Runtime::resume_session`, `SessionLedgerEntry`/`SessionLedgerMessage`, and `ModelRequest.history`; resume-required sessions fail closed for missing or unsupported stores before model/tool execution while stateless sessions may run with `RuntimeServices::stateless()`.
...
`clanker-message` splits stable SDK contracts from Clankers transcript compatibility records. `content`, `contracts`, `streaming`, `tool_result`, and `semantic_event` are the reusable message boundary; the non-default `transcript-compat` feature exposes the `transcript` module with `AgentMessage`, `MessageId`, random ID generation, wall-clock timestamps, bash records, branch/compaction summaries, and custom desktop history records for session/provider/controller adapters. The legacy `clanker_message::message::*` module remains only as a compatibility import path behind that same feature and is labeled unsupported/internal in the generated inventory.
...
Configuration, prompt, and skill sources are split into neutral service DTOs. `clankers-config::core` exposes display-neutral summaries (`NeutralSettingsSummary`, `NeutralKeymapConfig`, `PromptServiceConfig`, `SkillServiceConfig`) while TUI theme/keymap projection remains a desktop/display concern. `RuntimeBuilder::prompt_source_service` accepts host-owned `PromptSourceService` implementations; embedded defaults use static host context only, filesystem context fails closed unless policy enables it, and missing skill services return an unavailable error instead of probing global/project skill directories. Desktop Clankers wires explicit skill roots through `DesktopRuntimeServiceAdapters`.
...
| Runtime provider service | `ProviderRouterService::complete` returns `ProviderModelResponse` | Bridge neutral runtime requests to a product or desktop provider/router implementation with explicit auth and credential services | Return `Completed` with semantic `ProviderStreamEvent`s, content, usage, stop reason, and safe receipt metadata | Return `RetryableFailure`, `TerminalFailure`, or fail closed when provider/auth/credential services were not injected |
| Semantic events | `SemanticEvent` and `SemanticEventMetadata` | Project runtime, agent, daemon, TUI, attach, JSON, or product outputs from one safe event stream | Preserve ordered prompt/model/tool/usage/completion semantics | Redact secret-like metadata and keep display/protocol/provider DTOs out of reusable events |
| Prompt sources | `PromptSourceService` returns `PromptSources` | Provide system text, host context, filesystem context, context references, and skill snippets explicitly | Static or host-owned sources assemble into provenance-tracked prompt sections | Filesystem context fails closed when disabled; unsupported context references are reported as safe metadata |
| Skill resolution | `SkillStore::resolve` returns `SkillResolution` | Resolve requested skills from app-owned content or explicit desktop roots | Return resolved snippets with safe receipt counts | Missing embedded service returns unavailable without global/project discovery |
...
- `clankers-engine-host`: no optional features; depends on `clankers-engine`, `clankers-tool-host`, `clanker-message`, `serde`, `serde_json`, and `thiserror`. Its green `session_ledger` module owns `SessionLedgerEntry`, `SessionLedgerMessage`, replay metadata, neutral `SessionLedgerError`, and deterministic engine-message projection without runtime IDs, clocks, desktop stores, daemon protocol, database, or TUI crates.
...
- **Yellow**: app-edge crates such as `clankers-runtime`, daemon, MCP, ACP, runtime extension services, provider adapters, storage, or plugin boundaries may be composed by a product, but only behind a product-owned integration layer and not as transitive dependencies of generic SDK crates. Product-owned session stores and storage schemas are yellow app-edge concerns; model-visible history should use the green `clankers-engine-host::session_ledger` DTOs when a reusable shape is sufficient.
...
### 2026-06-04 session ledger green promotion

- affected entrypoint: `SessionLedgerEntry`, `SessionLedgerMessage`, `SessionLedgerRecord`, `SessionLedgerReplay`, replay helpers, and engine-message projection now live in `clankers-engine-host::session_ledger` and are reexported from the `clankers-engine-host` root.
- replacement or adapter change: `clankers-runtime` keeps compatibility type aliases/reexports, while product examples store model-visible history with `SessionLedgerMessage` and keep only store/receipt wrappers local to the product.
- validation command: `scripts/check-session-resume-brick.rs`, `scripts/check-session-ledger-boundary.rs`, and `scripts/check-embedded-agent-sdk.rs`.
...
- affected entrypoint: `EmbeddableEngine`, `EngineTurnRequest`, `EngineTurnResult`, `EngineTurnReceipt`, and the engine-host observation/port records now appear in the embedded SDK inventory.
- replacement or adapter change: no removal or replacement; embedders can keep using `reduce`/`run_engine_turn`, while runtime facade integrations may dogfood the typed engine-host path through these documented bricks.
- validation command: `scripts/check-embedded-agent-sdk.rs`.

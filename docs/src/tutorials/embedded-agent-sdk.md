...
The session-resume-brick evidence is fixture backed across two product-shaped stores and the reusable `SessionLedgerEntry` API. `examples/embedded-session-store/session-resume-evidence.json` pins the `embedded-session-store` and `embedded-product-workbench` DTO/store shapes, expected restored `EngineModelRequest` role/text order, missing-session errors, and forbidden shell dependencies. `scripts/check-session-resume-brick.rs` verifies that both examples keep product-owned session/message DTOs, preserve restored user/tool/assistant context into the follow-up model request, and avoid `clankers-session`, `clankers-db`, JSONL restore shells, daemon sockets, and TUI/session restore logic. `scripts/check-session-ledger-boundary.rs` inventories desktop compatibility adapters separately from neutral SDK ledger DTOs, including the daemon resume seed path that now projects desktop `AgentMessage` history through `SessionLedgerEntry` before narrowing to daemon seed messages. The runtime API now exposes `Runtime::resume_session`, neutral `SessionLedgerEntry`/`SessionLedgerMessage` DTOs, and `ModelRequest.history`; resume-required sessions fail closed for missing or unsupported stores before model/tool execution while stateless sessions may run with `RuntimeServices::stateless()`.

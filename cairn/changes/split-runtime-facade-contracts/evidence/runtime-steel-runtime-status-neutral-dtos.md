...
Moved reusable Steel runtime status/reason/outcome DTOs to neutral message contracts:

- Added `clanker_message::SteelRuntimeStatusCode`, `SteelRuntimeReasonCode`, and `SteelHostCallOutcome` with the existing wire shapes.
...
- Kept Steel runtime profile/request/receipt structs, host-call receipts, deterministic fixture evaluation, receipt hashing, budget enforcement, and fail-closed policy in `clankers-runtime`.

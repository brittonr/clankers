...
Moved the reusable Steel runtime status schema constant to neutral message contracts:

- Added `clanker_message::STEEL_RUNTIME_STATUS_SCHEMA` beside the neutral `SteelRuntimeStatus` DTO.
- Re-exported the constant through `clankers-runtime::steel_runtime` and the runtime crate root so existing public runtime paths remain source-compatible.
- Kept Steel evaluation, receipt hashing, budget enforcement, host-call admission, and receipt schema ownership in `clankers-runtime`.
...
TMPDIR=/home/brittonr/.cargo-target/tmp RUSTC_WRAPPER= cargo test -p clankers-runtime --lib status_does_not_claim_sandboxing

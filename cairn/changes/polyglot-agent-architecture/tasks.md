...
- [ ] [parallel] S2: Add hot-reload boundary tests proving a script change can alter routing decisions but cannot add host functions, enlarge budgets, or gain new capabilities without a profile/policy/UCAN change; current slices cover the "cannot add host function" fixture and ambient-access matrix, budget/capability enlargement still pending [r[polyglot-agent-architecture.steel-orchestration.hot-reload-boundary]]

...
- [ ] [serial] I8: Implement preflight for target classification, path normalization, path/symlink escape rejection, target hash capture, dirty-WIP policy, checkpoint/backup planning, policy hash capture, and approval state capture; current pure host preflight covers classification/path normalization, denied path patterns, target hash requirement, dirty-WIP checkpoint requirement, policy hash capture, and approval state capture, while real symlink checks and backup artifact creation remain for the imperative shell [r[steel-self-mutation-policy.receipts-and-preflight]]
- [ ] [parallel] I9: Emit deterministic redacted mutation receipts for allowed, denied, failed-verification, and rollback outcomes with Nickel policy hash and safe UCAN metadata only; current preflight receipts are deterministic/redacted and include policy hash, checkpoint plan, host function, safe UCAN metadata, and `writes_performed=false` for ready/blocked/denied pre-write outcomes [r[steel-self-mutation-policy.receipts-and-preflight.safe-receipt]]
...
- [x] [parallel] V1: Add deterministic positive fixtures for a bounded skill or prompt mutation where Nickel policy and UCAN authorization match and verification passes; current fixture authorizes a bounded prompt apply request through the Rust decision core and marks the host preflight receipt ready without writing [r[steel-self-mutation-policy.verification-fixtures.positive]]
- [ ] [parallel] V2: Add deterministic negative fixtures for path escape, missing/expired/wrong-resource UCAN, unauthorized verb, raw ambient write, failed verification, and stale rollback target; current policy/core/preflight fixtures cover path escape, unauthorized verb, ambient authority, missing/expired/revoked/wrong-audience/wrong-resource/wrong-verb/over-delegated UCAN, wildcard resources, missing approval, missing patch, missing UCAN requirement, missing receipt policy hash, missing session capability, disabled host function, dirty repository without checkpoint, and missing rollback target hash [r[steel-self-mutation-policy.verification-fixtures.negative]]

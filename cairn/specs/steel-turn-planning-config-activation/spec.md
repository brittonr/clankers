...
Clankers MUST provide a stable typed settings/configuration surface that can enable or disable Steel turn planning for real agent turns without code changes.

#### Scenario: absent config remains disabled [r[steel-turn-planning-config-activation.settings-surface.absent-disabled]]
- GIVEN no Steel turn planning configuration is present
...
#### Scenario: explicit config selects a reviewed profile [r[steel-turn-planning-config-activation.settings-surface.explicit-profile]]
- GIVEN settings name a reviewed Steel turn planning profile
- WHEN Clankers builds a real agent `TurnConfig`
- THEN Rust MUST attempt to load that profile through a typed activation helper
...
#### Scenario: valid profile loads bounded script material [r[steel-turn-planning-config-activation.profile-loader.valid]]
- GIVEN a profile declares `steel.host.plan_turn`, a supported rollout stage, a supported fallback mode, a bounded script path, and expected hashes
- WHEN Rust activates the profile
- THEN Rust MAY construct `AgentTurnSteelPlanningConfig`
- AND the config MUST contain only bounded planning source/payload metadata, session capability names, UCAN ability names, disabled-action policy, and the typed runtime profile

#### Scenario: invalid profile fails closed [r[steel-turn-planning-config-activation.profile-loader.invalid]]
- GIVEN the profile is missing, malformed, over budget, hash-mismatched, requests an unsupported seam, requests unsupported host actions, or points outside allowed policy/script roots
- WHEN Rust activates the profile
- THEN activation MUST fail before the turn starts
- AND no Steel script may run
- AND no fallback may grant broader authority than the disabled/no-config path

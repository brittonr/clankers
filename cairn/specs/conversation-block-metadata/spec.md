# conversation-block-metadata Specification

## Purpose
Define canonical timestamps and finalized BLAKE3 hashes for conversation blocks across live rendering, restore, attach replay, and other machine-readable consumers.

## Requirements
### Requirement: Each conversation block has a canonical timestamp [conversation.block.metadata.canonical.timestamp]
...
#### Scenario: Live block captures its opening timestamp [conversation.block.metadata.canonical.timestamp.live-block-opening-message-time]
...
#### Scenario: Restored history preserves original block time [conversation.block.metadata.canonical.timestamp.restore-preserves-original-time]
- GIVEN the scenario is evaluated
...
#### Scenario: Branched blocks get distinct timestamps [conversation.block.metadata.canonical.timestamp.branch-gets-distinct-time]
- GIVEN the scenario is evaluated
...
### Requirement: Each finalized conversation block has a canonical BLAKE3 hash [conversation.block.metadata.finalized.blake3.hash]
...
#### Scenario: Replay yields the same hash [conversation.block.metadata.finalized.blake3.hash.replay-of-same-sequence-matches]
- GIVEN the scenario is evaluated
...
#### Scenario: Content changes change the hash [conversation.block.metadata.finalized.blake3.hash.content-changes-change-digest]
- GIVEN the scenario is evaluated
...
#### Scenario: Ephemeral UI state does not affect the hash [conversation.block.metadata.finalized.blake3.hash.ui-state-does-not-change-digest]
- GIVEN the scenario is evaluated
...
### Requirement: Shared block surfaces expose consistent metadata [conversation.block.metadata.shared.surfaces]
...
#### Scenario: Machine-readable block consumers share canonical fields [conversation.block.metadata.shared.surfaces.machine-readable-consumers-share-fields]
- GIVEN the scenario is evaluated
...
#### Scenario: Standalone and attach mode agree on metadata [conversation.block.metadata.shared.surfaces.standalone-and-attach-agree]
- GIVEN the scenario is evaluated
...
#### Scenario: In-progress blocks defer final hash publication [conversation.block.metadata.shared.surfaces.streaming-blocks-defer-final-hash]
- GIVEN the scenario is evaluated

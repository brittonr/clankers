...
#### Scenario: engine crate is layered alongside clankers-core through explicit ownership r[embeddable-agent-engine.core-engine-explicit-layering]
...
The engine MUST define explicit host-facing contracts for model execution requests, tool execution requests, host feedback, and semantic engine events after an adapter has accepted any core-owned prompt lifecycle transition. Host adapters MUST be pure delegation wrappers; the engine-host boundary MUST NOT require adapters to own provider-specific streaming, request construction, or shared mutable conversation state. Host adapters MUST submit every accepted prompt lifecycle transition exactly once into the model/tool turn path, including the first prompt, ordinary subsequent user prompts, queued prompts, loop follow-ups, and controller-dispatched follow-ups. Dispatch acknowledgement MUST NOT be treated as prompt completion.
r[embeddable-agent-engine.host-driven-contracts]

#### Scenario: subsequent accepted user prompt streams in the same session r[embeddable-agent-engine.host-driven-contracts.subsequent-user-prompt-streams]
...
#### Scenario: dispatched follow-up waits for real prompt completion r[embeddable-agent-engine.host-driven-contracts.follow-up-completion-correlation]
...
#### Scenario: failed follow-up dispatch is visible and recoverable r[embeddable-agent-engine.host-driven-contracts.follow-up-dispatch-failure-visible]
...
#### Scenario: host submits an accepted prompt through engine input r[embeddable-agent-engine.accepted-prompt-engine-input]
- GIVEN the scenario is evaluated
...
#### Scenario: model execution is requested through explicit engine effects r[embeddable-agent-engine.model-execution-effects-retained]
- GIVEN the scenario is evaluated
...
#### Scenario: tool execution is requested through explicit engine effects r[embeddable-agent-engine.tool-execution-effects-retained]
- GIVEN the scenario is evaluated
...
#### Scenario: semantic engine events stay separate from lifecycle events r[embeddable-agent-engine.turn-events-lifecycle-events-separated]
- GIVEN the scenario is evaluated
...
#### Scenario: host adapters are pure delegation wrappers r[embeddable-agent-engine.host-adapters-pure-delegation]
- GIVEN the scenario is evaluated
...
#### Scenario: repeated accepted prompts allocate fresh model request correlation r[embeddable-agent-engine.turn-orchestration-owned-after-acceptance.repeated-prompt-correlation]
...
#### Scenario: accepted prompt to model to tool to continuation flow is engine-owned r[embeddable-agent-engine.accepted-prompt-turn-flow-owned]
- GIVEN the scenario is evaluated
...
#### Scenario: model completion schedules engine-owned tool work r[embeddable-agent-engine.model-completion-tool-state-retained]
- GIVEN the scenario is evaluated
...
#### Scenario: accepted prompt submission creates engine-owned pending model work r[embeddable-agent-engine.accepted-prompt-pending-model-work]
- GIVEN the scenario is evaluated
...
#### Scenario: stop reasons and continuation decisions are engine-owned r[embeddable-agent-engine.stop-continuation-policy-retained]
- GIVEN the scenario is evaluated
...
#### Scenario: tool-result ingestion is engine-owned r[embeddable-agent-engine.tool-result-ingestion-retained]
- GIVEN the scenario is evaluated
...
#### Scenario: host classification is input and engine retry authorization is policy r[embeddable-agent-engine.host-classification]
- GIVEN the scenario is evaluated
...
#### Scenario: retryable model failure schedules engine-owned retry work r[embeddable-agent-engine.retry-scheduling]
- GIVEN the scenario is evaluated
...
#### Scenario: retry budget scope resets on successful model feedback r[embeddable-agent-engine.retry-budget-reset]
- GIVEN the scenario is evaluated
...
#### Scenario: deterministic retry defaults preserve current behavior r[embeddable-agent-engine.retry-defaults]
- GIVEN the scenario is evaluated
...
#### Scenario: non-retryable model failure terminalizes through engine policy r[embeddable-agent-engine.non-retryable-terminalization]
- GIVEN the scenario is evaluated
...
#### Scenario: exhausted retry budget terminalizes through engine policy r[embeddable-agent-engine.retry-exhaustion-terminalization]
- GIVEN the scenario is evaluated
...
#### Scenario: model-continuation budget has explicit counting rules r[embeddable-agent-engine.model-continuation-budget]
- GIVEN the scenario is evaluated
...
#### Scenario: zero model-continuation budget is rejected before initial request r[embeddable-agent-engine.zero-budget]
- GIVEN the scenario is evaluated
...
#### Scenario: turn budget blocks unbounded continuations r[embeddable-agent-engine.budget-exhaustion]
- GIVEN the scenario is evaluated
...
#### Scenario: retry-wait cancellation clears scheduled retry work r[embeddable-agent-engine.retry-wait-cancellation]
- GIVEN the scenario is evaluated
...
#### Scenario: token-limit stop is explicit engine terminal behavior r[embeddable-agent-engine.max-tokens]
- GIVEN the scenario is evaluated
...
#### Scenario: retry and budget effect payloads stay engine-native r[embeddable-agent-engine.engine-native-payloads]
- GIVEN the scenario is evaluated
...
#### Scenario: reducer tests cover positive and negative retry policy paths r[embeddable-agent-engine.reducer-retry-tests]
- GIVEN the scenario is evaluated
...
#### Scenario: reducer tests cover turn budget and token-limit terminal paths r[embeddable-agent-engine.reducer-budget-token-tests]
- GIVEN the scenario is evaluated
- **WHEN** validation runs for this slice
...
#### Scenario: invalid retry feedback leaves state unchanged r[embeddable-agent-engine.invalid-retry-feedback]
- GIVEN the scenario is evaluated
...
#### Scenario: runtime adapter rails reject local policy re-derivation r[embeddable-agent-engine.adapter-rail]
- GIVEN the scenario is evaluated
...
#### Scenario: engine cargo tree excludes runtime provider graph r[embeddable-agent-engine.engine-cargo-tree-clean]
- GIVEN the scenario is evaluated
...
#### Scenario: message contracts do not depend on router runtime r[embeddable-agent-engine.message-without-router]
- GIVEN the scenario is evaluated
...
#### Scenario: router and provider consume canonical message contracts r[embeddable-agent-engine.router-provider-reexports]
- GIVEN the scenario is evaluated
...
#### Scenario: engine no longer filters shell message variants r[embeddable-agent-engine.no-agent-message-filtering]
- GIVEN the scenario is evaluated
...
#### Scenario: Clankers adapter owns transcript conversion r[embeddable-agent-engine.adapter-transcript-conversion]
- GIVEN the scenario is evaluated
...
#### Scenario: cargo-tree rail rejects forbidden transitive crates r[embeddable-agent-engine.cargo-tree-rail]
- GIVEN the scenario is evaluated
...
#### Scenario: source rail rejects forbidden public surface imports r[embeddable-agent-engine.source-surface-rail]
- GIVEN the scenario is evaluated
...
#### Scenario: core-owned lifecycle policy stays in clankers-core r[embeddable-agent-engine.core-owned-policy]
- GIVEN the scenario is evaluated
...
#### Scenario: engine-owned turn policy stays in clankers-engine r[embeddable-agent-engine.engine-owned-policy]
- GIVEN the scenario is evaluated
...
#### Scenario: engine state contains only active turn data r[embeddable-agent-engine.engine-state-active-data]
- GIVEN the scenario is evaluated
...
#### Scenario: adapter composition is explicit r[embeddable-agent-engine.explicit-adapter-composition]
- GIVEN the scenario is evaluated
...
#### Scenario: adapter-held prompt correlation replaces engine core state r[embeddable-agent-engine.adapter-held-prompt-correlation]
- GIVEN the scenario is evaluated
...
#### Scenario: cancellation phase ownership is explicit r[embeddable-agent-engine.cancellation-phase-ownership]
- GIVEN the scenario is evaluated
...
#### Scenario: engine dependency graph excludes clankers-core r[embeddable-agent-engine.engine-excludes-core-dependency]
- GIVEN the scenario is evaluated
- **WHEN** validation inspects normal dependencies for `clankers-engine`
...
#### Scenario: engine feedback construction moves to the host runner seam r[embeddable-agent-engine.host-feedback-construction-seam]
- GIVEN the scenario is evaluated
...
#### Scenario: cross-reducer source rail allows the host runner feedback seam r[embeddable-agent-engine.cross-reducer-source-rail]
- GIVEN the scenario is evaluated
...
#### Scenario: agent public APIs stay shell-native for core lifecycle r[embeddable-agent-engine.agent-core-type-rail]
- GIVEN the scenario is evaluated
...
#### Scenario: composition tests cover positive and negative sequencing r[embeddable-agent-engine.composition-tests]
- GIVEN the scenario is evaluated
...
#### Scenario: host runner drives engine effects through traits r[embeddable-agent-engine.host-runner-traits]
- GIVEN the scenario is evaluated
...
#### Scenario: Clankers agent becomes default assembly r[embeddable-agent-engine.agent-default-assembly]
- GIVEN the scenario is evaluated
...
#### Scenario: tool catalog and executor are independent host components r[embeddable-agent-engine.tool-host-catalog]
- GIVEN the scenario is evaluated
...
#### Scenario: tool-host outcomes and usage failures are verified r[embeddable-agent-engine.tool-host-outcome-verification]
- GIVEN the scenario is evaluated
...
#### Scenario: plugin-backed tools share the same executor seam r[embeddable-agent-engine.plugin-tool-adapter]
- GIVEN the scenario is evaluated
...
#### Scenario: stream folding handles normal model output r[embeddable-agent-engine.stream-folding-positive]
- GIVEN the scenario is evaluated
...
#### Scenario: stream folding rejects or normalizes malformed inputs deterministically r[embeddable-agent-engine.stream-folding-negative]
- GIVEN the scenario is evaluated
...
#### Scenario: source rails reject duplicated runner policy r[embeddable-agent-engine.no-duplicated-runner-policy]
- GIVEN the scenario is evaluated
...
#### Scenario: host crates stay dependency-light r[embeddable-agent-engine.host-crate-boundary-rails]
- GIVEN the scenario is evaluated
...
#### Scenario: runtime parity rails cover host adapters r[embeddable-agent-engine.host-adapter-parity]
- GIVEN the scenario is evaluated
...
#### Scenario: host crate artifact refresh is validated r[embeddable-agent-engine.host-artifact-refresh]
- GIVEN the scenario is evaluated
...
#### Scenario: embedder can identify the supported crate set r[embeddable-agent-engine.productized-sdk-surface.supported-crate-set]
- GIVEN the scenario is evaluated
...
#### Scenario: public entrypoints are inventoried r[embeddable-agent-engine.productized-sdk-surface.public-entrypoints-inventoried]
- GIVEN the scenario is evaluated
...
#### Scenario: example runs a prompt through fake adapters r[embeddable-agent-engine.external-consumer-example.fake-adapters]
- GIVEN the scenario is evaluated
...
#### Scenario: example dependency graph excludes Clankers shells r[embeddable-agent-engine.external-consumer-example.dependency-graph-clean]
- GIVEN the scenario is evaluated
...
#### Scenario: example public API avoids runtime-handle leakage r[embeddable-agent-engine.external-consumer-example.public-api-no-runtime-handles]
- GIVEN the scenario is evaluated
...
#### Scenario: adapter recipes cover successful and failing paths r[embeddable-agent-engine.adapter-recipes.positive-negative-paths]
- GIVEN the scenario is evaluated
...
#### Scenario: transcript conversion stays adapter-owned r[embeddable-agent-engine.adapter-recipes.transcript-conversion-owned-by-host]
- GIVEN the scenario is evaluated
...
#### Scenario: host runner depends on interfaces rather than implementations r[embeddable-agent-engine.adapter-only-modular-coupling.host-runner-traits]
- GIVEN the scenario is evaluated
...
#### Scenario: composition happens at application edge r[embeddable-agent-engine.adapter-only-modular-coupling.application-edge-composition]
- GIVEN the scenario is evaluated
...
#### Scenario: boundary rails reject tight coupling regressions r[embeddable-agent-engine.adapter-only-modular-coupling.tight-coupling-rail]
- GIVEN the scenario is evaluated
...
#### Scenario: versioning and migration policy is documented r[embeddable-agent-engine.sdk-support-policy.versioning-documented]
- GIVEN the scenario is evaluated
- **WHEN** a developer reads the embedded-agent SDK documentation
...
#### Scenario: support policy is checked against public API inventory r[embeddable-agent-engine.sdk-support-policy.inventory-classification]
- GIVEN the scenario is evaluated
...
#### Scenario: feature policy is documented r[embeddable-agent-engine.sdk-feature-default-policy.documented]
- GIVEN the scenario is evaluated
- **WHEN** a developer reads the embedded-agent SDK documentation
...
#### Scenario: feature policy is validated r[embeddable-agent-engine.sdk-feature-default-policy.validated]
- GIVEN the scenario is evaluated
...
#### Scenario: public API inventory is checked r[embeddable-agent-engine.embedding-api-stability-rails.public-api-inventory]
- GIVEN the scenario is evaluated
...
#### Scenario: dependency boundary stays clean r[embeddable-agent-engine.embedding-api-stability-rails.dependency-boundary-clean]
- GIVEN the scenario is evaluated
...
#### Scenario: acceptance bundle covers docs and executable examples r[embeddable-agent-engine.embedding-acceptance-bundle.docs-examples]
- GIVEN the scenario is evaluated
...
#### Scenario: acceptance bundle preserves existing Clankers behavior r[embeddable-agent-engine.embedding-acceptance-bundle.clankers-parity]
- GIVEN the scenario is evaluated
...
### Requirement: Confirmation broker kit fails closed and records safe decisions [r[embeddable-agent-engine.confirmation-broker-kit]]

The system MUST define `confirmation-broker-kit` as a composable Clankers brick with explicit ownership boundaries, deterministic fixtures, and safe evidence.

#### Scenario: Brick boundary is explicit [r[embeddable-agent-engine.confirmation-broker-kit.boundary]]

- GIVEN a product or contributor adopts the `confirmation-broker-kit` brick
...
#### Scenario: Brick has executable evidence [r[embeddable-agent-engine.confirmation-broker-kit.evidence]]
...
#### Scenario: Brick drift is diagnosable [r[embeddable-agent-engine.confirmation-broker-kit.drift]]

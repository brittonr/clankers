...
Self-evolution already has run, approval, application, and rollback receipts with safety gates. The pattern should be reusable as a safe artifact-promotion brick.

Evidence anchors: src/self_evolution/*; openspec/specs/self-evolution-control/spec.md.

## Goals / Non-Goals

**Goals:**
- Make `self-evolution-receipt-chain-kit` copyable, inspectable, and testable as a small brick.
...
**Choice:** Implement the smallest recipe, manifest, policy, generated inventory, or receipt validator that proves `self-evolution-receipt-chain-kit`.
...
- `openspec validate brick-09-self-evolution-receipt-chain-kit --strict --json`
- Focused Rust test/example/checker for `self-evolution-receipt-chain-kit`
- `git diff --check`

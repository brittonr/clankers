...
- [x] [serial] I1: Define CLI/session configuration for named Steel live-mutation runs, including default-deny behavior, visible mutation profile metadata, and separation from normal Steel eval/self-evolution candidate runs [r[steel-self-mutation-policy.explicit-opt-in]]
...
- [x] [serial] I11: Implement rollback guarded by recorded post-apply target hash and backup hash so operator edits are not clobbered [r[steel-self-mutation-policy.verification-and-rollback.guarded-rollback]]

## Verification and documentation

- [x] [parallel] V1: Add deterministic positive fixtures for a bounded skill or prompt mutation where Nickel policy and UCAN authorization match and verification passes; current fixture authorizes a bounded prompt apply request through the Rust decision core, marks the host preflight receipt ready, exercises an imperative full-replace apply shell that writes through a test target store, records target/backup/patch hashes, runs verification, emits a successful apply receipt, and rolls back through hash guards [r[steel-self-mutation-policy.verification-fixtures.positive]]
- [x] [parallel] V2: Add deterministic negative fixtures for path escape, missing/expired/wrong-resource UCAN, unauthorized verb, raw ambient write, failed verification, and stale rollback target; current policy/core/preflight/apply/rollback fixtures cover path escape, unauthorized verb, ambient authority, missing/expired/revoked/wrong-audience/wrong-resource/wrong-verb/over-delegated UCAN, wildcard resources, missing approval, missing patch, missing UCAN requirement, missing receipt policy hash, missing session capability, disabled host function, dirty repository without checkpoint, missing rollback target hash, stale target hash before write, patch hash mismatch, failed verification after write, stale rollback target, and backup hash mismatch [r[steel-self-mutation-policy.verification-fixtures.negative]]
...
- [x] [parallel] V4: Add Steel runtime tests proving mutation profiles still deny raw filesystem, shell, git, network, provider, credential, daemon, TUI, and native-tool access outside typed host functions [r[steel-self-mutation-policy.host-functions.raw-write-denied]]
- [x] [parallel] V5: Add receipt redaction tests proving compact UCAN tokens, raw proofs, credentials, provider payloads, oversized patch bodies, and uncontrolled absolute-path dumps are never emitted; current preflight/apply/rollback receipts use hashes/safe metadata, dynamic-runtime ambient denial fixtures redact attempted content, and Steel runtime receipts redact source/paths/credentials/oversized output [r[steel-self-mutation-policy.receipts-and-preflight.safe-receipt]]
- [x] [serial] D1: Document the operator workflow, policy review checklist, UCAN grant shape, approval tiers, verification expectations, receipt review, and rollback process [r[steel-self-mutation-policy.explicit-opt-in.named-run]]

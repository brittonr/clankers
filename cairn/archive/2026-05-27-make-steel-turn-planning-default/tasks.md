...
- [x] V3 [serial] r[steel-turn-planning-config-activation.profile-loader.invalid-bundled-default] Add negative tests for missing/malformed/hash-mismatched/over-budget default profile or script failure before Steel execution; missing bundled artifacts are documented as compile-time `include_*` failures in evidence. [evidence=evidence/steel-default-rails.md]

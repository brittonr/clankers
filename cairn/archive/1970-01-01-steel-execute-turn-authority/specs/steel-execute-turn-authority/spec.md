# Steel Execute Turn Authority Delta

## Purpose

Adds an explicit `steel.host.execute_turn` authority contract for default Steel-selected execution so the planning grant does not implicitly authorize provider/tool host-runner work.

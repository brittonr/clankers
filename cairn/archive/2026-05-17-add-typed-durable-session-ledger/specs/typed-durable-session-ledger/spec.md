...
The system MUST persist structured, versioned session facts for agent execution while preserving existing human-readable transcript and JSONL compatibility.
...
- GIVEN a session records user input, model request, model output, tool call, tool result, TUI block, review finding, or OpenSpec task event
- WHEN the session persistence layer records the event
- THEN it writes a typed ledger fact with record kind, schema version, safe identifiers, and relevant artifact hashes
...
- GIVEN an event contains credentials, headers, environment values, raw provider request bodies, or unredacted tool output
- WHEN the typed ledger fact is written
- THEN secret-bearing fields are omitted, redacted, or replaced with safe artifact references according to the record schema
